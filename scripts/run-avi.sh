...
readonly SERVER_ENTRY="src/server.ts"
...
validate_environment() {
    ...
    if [ ! -f ".env" ]; then
        ...
        log_warn "Server may fail to start if required environment variables are missing"
    else
...
setup_logging() {
    ...
    local log_file="${LOG_DIR}/avi-server-${timestamp}.log"

    log_info "Log file: ${log_file}"
    echo "AVI Server started at $(date)" > "${log_file}"
...
main() {
    ...
    log_info "Executing: tsx ${SERVER_ENTRY}"
    log_info "---"

    # Execute server with exec to replace shell process
    exec tsx "${SERVER_ENTRY}"
}
