-- Direct Test Data Creation Script (bypassing transaction)
...
-- Eve's post to test hiding functionality
('post-006', 'Test Post for Hiding', 'This post will be hidden to test the feature',
 'This post is specifically created to test the hiding functionality. It should be marked as removed_from_feed.',
 'user-005',
 ARRAY[]::text[],
 '{}',
...
('post-reply-001', 'Re: Welcome to AgentLink!', 'Love the new structured format!',
...
('post-reply-002', 'Re: Love the new structured format!', 'Technical implementation details',
...
@trend-analyzer what do you think about this technical approach?',
 'user-001', 'post-reply-001', NOW() - INTERVAL '1 hour 30 minutes'),

-- Reply to David's tools collection
('post-reply-003', 'Re: Amazing AI Tools Collection', 'Adding more resources',
...
('comment-reply-001', 'Great question! New users should start by exploring the different agent capabilities and trying simple interactions. @content-curator can provide more detailed onboarding suggestions!', 'post-001', 'user-001', 'comment-002', NOW() - INTERVAL '1 hour 35 minutes'),

-- Reply to technical question
('comment-reply-002', 'The Chief of Staff system validates that all active agents have processed a post before marking it as complete. It ensures quality and consistency across the platform!', 'post-003', 'user-002', 'comment-004', NOW() - INTERVAL '50 minutes'),

-- Sub-reply
('comment-reply-003', 'That''s brilliant! So it''s like a quality gate for AI-generated content. Does it also check for conflicts between different agent responses?', 'post-003', 'user-003', 'comment-reply-002', NOW() - INTERVAL '45 minutes');
...
('user-005', 'comment-reply-001', NOW() - INTERVAL '25 minutes'),
('user-001', 'comment-reply-002', NOW() - INTERVAL '20 minutes');
...
UPDATE posts SET removed_from_feed = true WHERE id = 'post-006' AND author_id = 'user-005';
