...
const SSEEventStreamer_1 = require("../../services/SSEEventStreamer");
...
router.get('/status', (req, res) => {
    try {
        const basicInfo = EnhancedProcessManager_1.enhancedProcessManager.getProcessInfo();
        const enhancedInfo = EnhancedProcessManager_1.enhancedProcessManager.getEnhancedProcessInfo();
        const healthReport = EnhancedProcessManager_1.enhancedProcessManager.getNLDHealthReport();
        res.json({
            success: true,
            data: {
                basic: basicInfo,
...
router.post('/launch', async (req, res) => {
    try {
        ...
        logger_1.logger.info('Launching enhanced Claude instance', { instanceId, config });
        const processInfo = await EnhancedProcessManager_1.enhancedProcessManager.createInstance(instanceId, config);
        res.json({
            success: true,
            data: {
                instance: {
        ...
        logger_1.logger.error('Failed to launch enhanced Claude instance', error);
        res.status(500).json({
            success: false,
            error: error.message,
            timestamp: new Date().toISOString()
...
router.post('/kill', async (req, res) => {
    try {
        ...
        await EnhancedProcessManager_1.enhancedProcessManager.killInstance();
        res.json({
            success: true,
...
router.post('/restart', async (req, res) => {
    try {
        ...
        const processInfo = await EnhancedProcessManager_1.enhancedProcessManager.restartInstance();
        res.json({
            success: true,
            data: processInfo,
...
router.post('/input', async (req, res) => {
    try {
        ...
        const success = await EnhancedProcessManager_1.enhancedProcessManager.sendInput(instanceId, input);
        if (!success) {
        ...
        logger_1.logger.error('Failed to send input to enhanced Claude instance', error);
        res.status(500).json({
            success: false,
            error: error.message,
            timestamp: new Date().toISOString()
...
router.get('/:instanceId/terminal/stream', (req, res) => {
    try {
        ...
        logger_1.logger.info(`Creating terminal SSE stream: ${instanceId}`);
        // Check if instance exists
        const processInfo = EnhancedProcessManager_1.enhancedProcessManager.getInstanceInfo(instanceId);
        if (!processInfo) {
            return res.status(404).json({
                success: false,
        ...
        // Create SSE stream
        const connectionId = SSEEventStreamer_1.sseEventStreamer.createTerminalStream(instanceId, res);
        logger_1.logger.info(`Terminal SSE stream created: ${instanceId}`, { connectionId });
    }
    catch (error) {
        logger_1.logger.error(`Failed to create terminal stream for ${req.params.instanceId}`, error);
        if (!res.headersSent) {
            res.status(500).json({
                success: false,
                error: error.message,
...
router.get('/status/stream', (req, res) => {
    try {
        logger_1.logger.info('Creating status SSE stream');
        const connectionId = SSEEventStreamer_1.sseEventStreamer.createStatusStream(res);
        logger_1.logger.info('Status SSE stream created', { connectionId });
    }
    catch (error) {
        logger_1.logger.error('Failed to create status stream', error);
        if (!res.headersSent) {
            res.status(500).json({
                success: false,
                error: error.message,
...
router.post('/:instanceId/terminal/resize', async (req, res) => {
    try {
        ...
        const success = await EnhancedProcessManager_1.enhancedProcessManager.resizeTerminal(instanceId, cols, rows);
        if (!success) {
            return res.status(404).json({
                success: false,
        ...
        logger_1.logger.error(`Failed to resize terminal for instance ${req.params.instanceId}`, error);
        res.status(500).json({
            success: false,
            error: error.message,
            timestamp: new Date().toISOString()
...
router.get('/:instanceId/output', (req, res) => {
    try {
        const { instanceId } = req.params;
        const fromPosition = parseInt(req.query.fromPosition) || 0;
        ...
        res.json({
            ...
            filtered: EnhancedProcessManager_1.EscapeSequenceFilter.containsProblematicSequences(output),
            instanceId,
            timestamp: new Date().toISOString()
        });
    }
    catch (error) {
        logger_1.logger.error(`Failed to get output for instance ${req.params.instanceId}`, error);
        res.status(500).json({
            success: false,
            error: error.message,
            timestamp: new Date().toISOString()
...
router.put('/config', (req, res) => {
    try {
        ...
        EnhancedProcessManager_1.enhancedProcessManager.updateConfig(config);
        res.json({
            success: true,
            message: 'Configuration updated successfully',
            data: EnhancedProcessManager_1.enhancedProcessManager.getProcessInfo(),
            timestamp: new Date().toISOString()
        });
    }
    catch (error) {
...
router.get('/nld/health', (req, res) => {
    try {
        ...
        const instanceHealthReport = EnhancedProcessManager_1.enhancedProcessManager.getNLDHealthReport();
        res.json({
            success: true,
            data: {
...
router.get('/nld/alerts', (req, res) => {
    try {
        ...
        let alerts = nldProcessMonitor.getAlertHistory(pattern);
...
function getPatternResolution(pattern) {
    const resolutions = {
...
//# sourceMappingURL=processManagerEnhanced.js.map