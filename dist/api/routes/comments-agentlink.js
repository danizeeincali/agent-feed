...
router.post('/posts/:postId/comments', validateCommentData, async (req, res) => {
    try {
        const { postId } = req.params;
        const { content, authorAgent, parentId, metadata = {} } = req.body;
        if (!postId) {
            return res.status(400).json({
                success: false,
        ...
        // Verify post exists
        const postCheck = await connection_1.db.query('SELECT id FROM posts WHERE id = $1 AND status = \'published\'', [postId]);
        ...
        let parentDepth = 0;
        if (parentId) {
            const parentCheck = await connection_1.db.query('SELECT id, thread_depth FROM comments WHERE id = $1 AND post_id = $2 AND status = \'published\'', [parentId, postId]);
    ...
    `;
        ...
        logger_1.logger.info('Comment created successfully', { commentId, postId, authorAgent, parentId });
        res.status(201).json({
            success: true,
        ...
        logger_1.logger.error('Failed to create comment', { error, postId: req.params.postId });
...
router.get('/posts/:postId/comments', async (req, res) => {
    try {
        const { postId } = req.params;
        const { sort = 'newest', authorAgent, limit = '50', offset = '0', includeReplies = 'true' } = req.query;
        if (!postId) {
            return res.status(400).json({
                success: false,
                error: 'Post ID is required'
        ...
        // Verify post exists
        const postCheck = await connection_1.db.query('SELECT id FROM posts WHERE id = $1', [postId]);
        if (postCheck.rows.length === 0) {
            return res.status(404).json({
                success: false,
                error: 'Post not found'
    ...
    `;
        const metaResult = await connection_1.db.query(metaQuery, [postId]);
        ...
        logger_1.logger.error('Failed to fetch comments', { error, postId: req.params.postId });
        res.status(500).json({
            success: false,
...
router.post('/comments/:id/like', async (req, res) => {
    try {
        ...
        const commentCheck = await connection_1.db.query('SELECT id FROM comments WHERE id = $1 AND status = \'published\'', [id]);
        ...
        if (existingLike.rows.length > 0) {
            // Remove like
            await connection_1.db.query('DELETE FROM comment_likes WHERE id = $1', [existingLike.rows[0].id]);
        }
        else {
            // Add like
      ...
      `;
        ...
        // Get updated count
        const countResult = await connection_1.db.query('SELECT like_count FROM comments WHERE id = $1', [id]);
        ...
        logger_1.logger.error('Failed to toggle comment like', { error, commentId: req.params.id });
        res.status(500).json({
            success: false,
...
//# sourceMappingURL=comments-agentlink.js.map