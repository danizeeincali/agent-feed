...
const validateSearch = [
    (0, express_validator_1.body)('query').optional().isString().withMessage('Query must be a string'),
    (0, express_validator_1.body)('filters.agents').optional().isArray().withMessage('Agents filter must be an array'),
    (0, express_validator_1.body)('filters.tags').optional().isArray().withMessage('Tags filter must be an array'),
    (0, express_validator_1.body)('filters.businessImpact.min').optional().isInt({ min: 0, max: 10 }).withMessage('Business impact min must be 0-10'),
    (0, express_validator_1.body)('filters.businessImpact.max').optional().isInt({ min: 0, max: 10 }).withMessage('Business impact max must be 0-10'),
    (0, express_validator_1.body)('filters.dateRange.start').optional().isISO8601().withMessage('Start date must be valid ISO8601'),
    (0, express_validator_1.body)('filters.dateRange.end').optional().isISO8601().withMessage('End date must be valid ISO8601'),
    (0, express_validator_1.body)('sort.field').optional().isIn(['publishedAt', 'businessImpact', 'relevance']).withMessage('Invalid sort field'),
    (0, express_validator_1.body)('sort.order').optional().isIn(['asc', 'desc']).withMessage('Sort order must be asc or desc'),
    (0, express_validator_1.body)('pagination.limit').optional().isInt({ min: 1, max: 100 }).withMessage('Limit must be 1-100'),
    (0, express_validator_1.body)('pagination.offset').optional().isInt({ min: 0 }).withMessage('Offset must be >= 0')
];
// Advanced post search with filters and full-text search
router.post('/posts', validateSearch, (req, res, next) => {
    ...
    const { query: searchQuery = '', filters = {}, sort = { field: 'publishedAt', order: 'desc' }, pagination = { limit: 20, offset: 0 }, searchOptions = {}, facets = [], trackSearch = false } = req.body;
    ...
    try {
      ...
      `;
        ...
        const result = await connection_1.db.query(sql, params);
      ...
      `;
        ...
        const countResult = await connection_1.db.query(countSql, countParams);
...
router.get('/suggestions', (0, express_validator_1.query)('q').optional().isString().withMessage('Query must be a string'), (0, express_validator_1.query)('type').optional().isIn(['query', 'agent', 'tag']).withMessage('Invalid suggestion type'), (0, error_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        if (!type || type === 'agent') {
          ...
          LIMIT 5
        `;
            const agentResult = await connection_1.db.query(agentQuery, [`%${q}%`]);
        ...
        if (!type || type === 'tag') {
          ...
          LIMIT 10
        `;
            const tagResult = await connection_1.db.query(tagQuery, [`%${q}%`]);
        ...
        logger_1.logger.error('Failed to get suggestions', {
            error: error instanceof Error ? error.message : 'Unknown error',
            query: q,
            type
        });
...
// Get trending topics
router.get('/trending', (0, express_validator_1.query)('period').optional().isIn(['1h', '24h', '7d', '30d']).withMessage('Invalid period'), (0, error_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        LIMIT 20
      `;
        const result = await connection_1.db.query(trendingQuery);
        ...
        logger_1.logger.error('Failed to get trending topics', {
            error: error instanceof Error ? error.message : 'Unknown error',
            period
        });
        res.status(500).json({
...
router.get('/analytics', (0, express_validator_1.query)('period').optional().isIn(['1d', '7d', '30d']).withMessage('Invalid period'), (0, error_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        logger_1.logger.error('Failed to get search analytics', {
            error: error instanceof Error ? error.message : 'Unknown error',
            period
        });
        res.status(500).json({
...
async function generateFacets(facets, filters, searchQuery) {
    const facetResults = {};
    for (const facet of facets) {
        try {
            ...
            LIMIT 20
          `;
                    const agentsResult = await connection_1.db.query(agentsQuery);
            ...
            LIMIT 30
          `;
                    const tagsResult = await connection_1.db.query(tagsQuery);
            ...
            ORDER BY count DESC
          `;
                    const impactResult = await connection_1.db.query(impactQuery);
            ...
            logger_1.logger.error(`Failed to generate facet: ${facet}`, error);
        }
    }
    return facetResults;
}
// Helper function to track search queries
async function trackSearchQuery(query, filters, resultCount) {
