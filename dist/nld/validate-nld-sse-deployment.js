...
exports.NLDSSEDeploymentValidator = void 0;
const fs_1 = require("fs");
const path_1 = require("path");
const sse_buffer_accumulation_detector_1 = __importDefault(require("./sse-buffer-accumulation-detector"));
const sse_event_handler_duplication_analyzer_1 = __importDefault(require("./sse-event-handler-duplication-analyzer"));
const output_buffer_management_failure_patterns_1 = __importDefault(require("./output-buffer-management-failure-patterns"));
const frontend_message_state_accumulation_detector_1 = __importDefault(require("./frontend-message-state-accumulation-detector"));
...
const sse_streaming_anti_patterns_database_1 = __importDefault(require("./sse-streaming-anti-patterns-database"));
const real_time_sse_failure_monitor_1 = __importDefault(require("./real-time-sse-failure-monitor"));
class NLDSSEDeploymentValidator {
    validationDir;
    bufferDetector;
    handlerAnalyzer;
     ...
     */
    async initializeAndValidateComponents() {
        const results = [];
        const validationId = `nld-sse-validation-${Date.now()}`;
        console.log('🔍 Starting NLD SSE Pattern Detection Deployment Validation...');
        // Initialize components
        try {
            this.bufferDetector = new sse_buffer_accumulation_detector_1.default(this.validationDir);
            results.push({
        ...
        try {
            this.handlerAnalyzer = new sse_event_handler_duplication_analyzer_1.default(this.validationDir);
            results.push({
        ...
        try {
            this.bufferFailureAnalyzer = new output_buffer_management_failure_patterns_1.default(this.validationDir);
            results.push({
        ...
        try {
            this.frontendDetector = new frontend_message_state_accumulation_detector_1.default(this.validationDir);
            results.push({
        ...
        try {
            this.tddStrategies = new tdd_sse_prevention_strategies_1.default(this.validationDir);
            results.push({
        ...
        if (this.bufferDetector && this.handlerAnalyzer && this.bufferFailureAnalyzer && this.frontendDetector) {
            try {
                this.neuralExporter = new sse_neural_training_export_1.default((0, path_1.join)(this.validationDir, 'neural-training'), this.bufferDetector, this.handlerAnalyzer, this.bufferFailureAnalyzer, this.frontendDetector);
                results.push({
                    component: 'SSENeuralTrainingExport',
                    status: 'pass',
                    message: 'Successfully initialized neural training export system',
                    details: { exportDir: (0, path_1.join)(this.validationDir, 'neural-training') }
                });
            }
            catch (error) {
                results.push({
        ...
        try {
            this.antiPatternsDB = new sse_streaming_anti_patterns_database_1.default(this.validationDir);
            if (this.bufferDetector && this.handlerAnalyzer && this.bufferFailureAnalyzer && this.frontendDetector) {
                this.antiPatternsDB.initializeAnalyzers(this.bufferDetector, this.handlerAnalyzer, this.bufferFailureAnalyzer, this.frontendDetector);
            }
            results.push({
        ...
        try {
            this.failureMonitor = new real_time_sse_failure_monitor_1.default();
            results.push({
    ...
    async runFunctionalTests() {
        ...
        if (this.handlerAnalyzer) {
            try {
                ...
                this.handlerAnalyzer.recordEventHandlerRegistration('onSSEMessage', 'validation-test', 'message', 'test-stack-1');
                this.handlerAnalyzer.recordEventHandlerRegistration('onSSEMessage', 'validation-test', 'message', 'test-stack-2');
        ...
        if (this.frontendDetector) {
            try {
                ...
                this.frontendDetector.trackMessageStateUpdate('TestComponent', 'validation-test', testMessages, 1);
     ...
     * Generate validation report
     */
    generateValidationReport(validationId, results) {
     ...
     */
    generateNextSteps(overallStatus, failedChecks, warningChecks) {
        const steps = [];
    ...
    persistValidationReport(report) {
        try {
            const reportFile = (0, path_1.join)(this.validationDir, `validation-report-${report.validationId}.json`);
            (0, fs_1.writeFileSync)(reportFile, JSON.stringify(report, null, 2));
            // Also create a latest report link
            const latestReportFile = (0, path_1.join)(this.validationDir, 'latest-validation-report.json');
            (0, fs_1.writeFileSync)(latestReportFile, JSON.stringify(report, null, 2));
     ...
     */
    async startProductionMonitoringIfValid() {
    ...
    getLatestValidationReport() {
        try {
            const latestReportFile = (0, path_1.join)(this.validationDir, 'latest-validation-report.json');
            if ((0, fs_1.existsSync)(latestReportFile)) {
                const data = (0, fs_1.readFileSync)(latestReportFile, 'utf8');
...
exports.NLDSSEDeploymentValidator = NLDSSEDeploymentValidator;
exports.default = NLDSSEDeploymentValidator;
//# sourceMappingURL=validate-nld-sse-deployment.js.map