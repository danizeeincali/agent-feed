...
exports.TDDTerminalStormPreventionStrategies = void 0;
const events_1 = require("events");
const fs_1 = require("fs");
const path_1 = require("path");
class TDDTerminalStormPreventionStrategies extends events_1.EventEmitter {
    strategies = [];
    testCaseLibrary = [];
    storageFile;
    constructor(storageDir) {
        super();
        this.storageFile = (0, path_1.join)(storageDir, 'tdd-terminal-storm-prevention.json');
...
describe('Output Buffer Management', () => {
  ...
  it('should maintain recent output when buffer is full', () => {
     ...
     */
    getStrategiesForPattern(pattern) {
        return this.strategies.filter(s => s.targetPattern.includes(pattern) ||
            s.tddTestCases.some(tc => tc.relatedPatterns.includes(pattern)));
    }
    /**
     * Generate TDD test suite for pattern
     */
    generateTDDTestSuite(pattern) {
     ...
     */
    generateGenericTestSuite(pattern) {
...
describe('Generic Terminal Storm Prevention for ${pattern}', () => {
  ...
  it('should provide recovery mechanism', async () => {
     ...
     */
    validateStrategyEffectiveness(strategyId, metrics) {
     ...
     */
    getTestCasesByPriority(priority) {
    ...
    generatePreventionReport() {
        ...
        const highPriorityTests = this.getTestCasesByPriority('critical').concat(this.getTestCasesByPriority('high'));
    ...
    loadExistingStrategies() {
        try {
            if ((0, fs_1.existsSync)(this.storageFile)) {
                const data = (0, fs_1.readFileSync)(this.storageFile, 'utf8');
                const parsed = JSON.parse(data);
                // Load custom strategies (built-in ones are already initialized)
                if (parsed.customStrategies) {
                    parsed.customStrategies.forEach((strategy) => {
                        this.strategies.push(strategy);
                        strategy.tddTestCases.forEach(testCase => {
                            this.testCaseLibrary.push(testCase);
                        });
    ...
    persistStrategies() {
        try {
            ...
            const customStrategies = this.strategies.filter(s => !['pty-config-management', 'process-spawn-debouncing', 'sse-connection-deduplication', 'terminal-buffer-management'].includes(s.strategyId));
            const data = {
                customStrategies,
                lastUpdated: new Date().toISOString()
            };
            (0, fs_1.writeFileSync)(this.storageFile, JSON.stringify(data, null, 2));
        }
        catch (error) {
...
exports.TDDTerminalStormPreventionStrategies = TDDTerminalStormPreventionStrategies;
exports.default = TDDTerminalStormPreventionStrategies;
//# sourceMappingURL=tdd-terminal-storm-prevention-strategies.js.map