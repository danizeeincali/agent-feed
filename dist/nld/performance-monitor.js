...
exports.NLDPerformanceMonitor = void 0;
const events_1 = require("events");
class NLDPerformanceMonitor extends events_1.EventEmitter {
    config;
    metrics = new Map();
    thresholds = new Map();
    alerts = [];
     ...
     */
    recordMetric(type, category, value, metadata = {}, tags = []) {
        const metric = {
     ...
     */
    getMetrics(type, category, timeRange) {
        const now = Date.now();
     ...
     */
    generatePerformanceReport(duration = 3600000) {
        const now = Date.now();
    ...
    getDashboardData() {
        ...
        return {
            ...
            performance_score: this.calculateOverallHealth(this.calculateKeyMetrics(hourlyMetrics), alerts)
        };
    }
    ...
    analyzeThresholds() {
        ...
        for (const [key, threshold] of this.thresholds) {
            const recentMetrics = this.getRecentMetrics(this.metrics.get(threshold.metric) || [], 60000 // Last minute
            );
    ...
    getRecentMetrics(metrics, timeRange) {
    ...
    calculateOverallHealth(keyMetrics, alerts) {
        let health = 1.0;
        // Deduct for alerts
    ...
    generateRecommendations(keyMetrics, trends, alerts) {
    ...
    getThresholdRecommendations(metric, severity) {
        const recommendations = [];
        switch (metric) {
    ...
    calculateSuccessRate(metrics, type) {
        const connectionMetrics = metrics.filter(m => m.type === type);
        if (connectionMetrics.length === 0)
            return 0;
        const successCount = connectionMetrics.filter(m => m.category === 'success' || m.value === 1).length;
    ...
    getNeuralTrainingProgress(metrics) {
        const neuralMetrics = metrics.filter(m => m.type === 'neural');
        if (neuralMetrics.length === 0)
            return 0;
    ...
    countMetricsByCategory(metrics, category) {
...
exports.NLDPerformanceMonitor = NLDPerformanceMonitor;
