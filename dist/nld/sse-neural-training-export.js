...
exports.SSENeuralTrainingExport = void 0;
const events_1 = require("events");
const fs_1 = require("fs");
const path_1 = require("path");
class SSENeuralTrainingExport extends events_1.EventEmitter {
    exportDir;
    datasets = new Map();
    bufferDetector;
    handlerAnalyzer;
    ...
    frontendDetector;
    constructor(exportDir, bufferDetector, handlerAnalyzer, bufferFailureAnalyzer, frontendDetector) {
        super();
     ...
     */
    exportSSEBufferReplayLoopPatterns() {
        const patterns = this.bufferDetector.getDetectedPatterns();
        const replayLoopPatterns = patterns.filter(p => p.antiPattern === 'SSE_BUFFER_REPLAY_LOOP' ||
            p.antiPattern === 'SSE_INFINITE_MESSAGE_REPETITION');
        const samples = replayLoopPatterns.map(pattern => ({
     ...
     */
    exportOutputPositionTrackingFailures() {
        const bufferFailures = this.bufferFailureAnalyzer.getBufferFailures();
        const positionFailures = bufferFailures.filter(f => f.failureType === 'position_reset' ||
            f.failureType === 'parser_corruption');
        const samples = positionFailures.map(failure => ({
     ...
     */
    exportFrontendMessageAccumulationPatterns() {
        const patterns = this.frontendDetector.getDetectedPatterns();
        const samples = patterns.map(pattern => ({
     ...
     */
    exportClaudeOutputParserFailures() {
        const parserFailures = this.bufferFailureAnalyzer.getParserFailures();
        const samples = parserFailures.map(failure => ({
     ...
     */
    generateCombinedAntiPatternDataset() {
     ...
     */
    generateNegativeExamples(antiPatternType, count) {
     ...
     */
    exportToTensorFlowJS(datasetId) {
        const dataset = this.datasets.get(datasetId);
        if (!dataset) {
            throw new Error(`Dataset ${datasetId} not found`);
        }
        ...
        const tfExportPath = (0, path_1.join)(this.exportDir, `${datasetId}-tensorflow.json`);
        (0, fs_1.writeFileSync)(tfExportPath, JSON.stringify(tfData, null, 2));
        console.log(`🤖 Exported TensorFlow.js dataset: ${tfExportPath}`);
    }
    /**
     * Export dataset in PyTorch format
     */
    exportToPyTorch(datasetId) {
        const dataset = this.datasets.get(datasetId);
        if (!dataset) {
            throw new Error(`Dataset ${datasetId} not found`);
        }
        ...
        const pytorchExportPath = (0, path_1.join)(this.exportDir, `${datasetId}-pytorch.json`);
        (0, fs_1.writeFileSync)(pytorchExportPath, JSON.stringify(pytorchData, null, 2));
        console.log(`🐍 Exported PyTorch dataset: ${pytorchExportPath}`);
    }
    /**
     * Generate training script for the dataset
     */
    generateTrainingScript(datasetId, framework) {
        const dataset = this.datasets.get(datasetId);
        if (!dataset) {
            throw new Error(`Dataset ${datasetId} not found`);
        }
...
if __name__ == '__main__':
    ...
    persistDataset(dataset) {
        try {
            const filePath = (0, path_1.join)(this.exportDir, `${dataset.datasetId}.json`);
            (0, fs_1.writeFileSync)(filePath, JSON.stringify(dataset, null, 2));
            ...
            (0, fs_1.writeFileSync)((0, path_1.join)(this.exportDir, `${dataset.datasetId}-train.js`), tfScript);
            (0, fs_1.writeFileSync)((0, path_1.join)(this.exportDir, `${dataset.datasetId}-train.py`), pytorchScript);
        }
        catch (error) {
...
exports.SSENeuralTrainingExport = SSENeuralTrainingExport;
exports.default = SSENeuralTrainingExport;
//# sourceMappingURL=sse-neural-training-export.js.map