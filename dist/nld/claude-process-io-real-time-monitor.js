...
exports.claudeProcessIOMonitor = exports.ClaudeProcessIORealTimeMonitor = void 0;
...
class ClaudeProcessIORealTimeMonitor {
    config;
    ...
    setupPatternDetectionCallbacks() {
        claude_process_io_failure_detector_1.claudeProcessIODetector.onPatternDetected((pattern, metrics) => {
    ...
    performPeriodicChecks() {
        const activeProcesses = claude_process_io_failure_detector_1.claudeProcessIODetector.getAllActiveProcesses();
        activeProcesses.forEach(metrics => {
            // Check for processes that might need attention
            this.checkProcessHealth(metrics);
        });
    }
    checkProcessHealth(metrics) {
    ...
    createHealthAlert(metrics, category, description) {
        // Check if we already have an alert for this pattern recently
        const instanceAlerts = this.activeAlerts.get(metrics.instanceId) || [];
        const recentAlert = instanceAlerts.find(alert => alert.pattern.category === category &&
    ...
    handlePatternDetection(pattern, metrics) {
        const alert = {
        ...
        if (this.config.neuralTraining) {
            ...
            claude_process_io_neural_training_dataset_1.claudeProcessIONeuralDataset.addProcessSession(metrics, [pattern], {
    ...
    generateAutomatedActions(pattern, metrics) {
        const actions = [];
    ...
    generateHumanActions(pattern, metrics) {
        const actions = [];
        switch (pattern.category) {
            case 'PRINT_FLAG_INPUT_REQUIRED':
    ...
    async executeAutomatedRecovery(alert) {
        ...
        try {
            ...
            if (this.config.neuralTraining) {
                claude_process_io_neural_training_dataset_1.claudeProcessIONeuralDataset.addProcessSession(alert.metrics, [alert.pattern], {
    ...
    getResolutionSuggestions(category, metrics) {
        const suggestions = [];
        switch (category) {
            case 'PRINT_FLAG_INPUT_REQUIRED':
    ...
    getMonitoringStatus() {
        ...
        return {
            ...
            activeProcesses: claude_process_io_failure_detector_1.claudeProcessIODetector.getAllActiveProcesses().length,
    ...
    generateSystemReport() {
        ...
        const neuralDatasetStats = {
            recordCount: claude_process_io_neural_training_dataset_1.claudeProcessIONeuralDataset.getRecordCount(),
            patternStats: claude_process_io_neural_training_dataset_1.claudeProcessIONeuralDataset.getPatternStatistics()
        };
...
exports.ClaudeProcessIORealTimeMonitor = ClaudeProcessIORealTimeMonitor;
// Export singleton instance
exports.claudeProcessIOMonitor = new ClaudeProcessIORealTimeMonitor();
//# sourceMappingURL=claude-process-io-real-time-monitor.js.map