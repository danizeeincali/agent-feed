...
exports.automatedRecoverySystem = exports.AutomatedRecoverySystem = void 0;
...
class AutomatedRecoverySystem extends events_1.EventEmitter {
    snapshots = new Map();
    recoveryAttempts = [];
    ...
    initializeRecoveryActions() {
        ...
        this.recoveryActions.set('CLAUDE_FUNCTIONALITY_REPAIR', {
            ...
            execute: async () => {
                try {
                    return await claude_functionality_validator_1.claudeFunctionalityValidator.repairClaudeFunctionality();
                }
                catch (error) {
            ...
            execute: async () => {
                try {
                    ...
                    for (const instanceId of activeConnections) {
                        const success = await sse_streaming_guardian_1.sseStreamingGuardian.attemptStreamingRecovery(instanceId);
                        if (success)
                            successCount++;
                    }
        ...
        this.recoveryActions.set('COMPONENT_STATE_RESET', {
            ...
            execute: async () => {
                try {
                    ...
                    component_state_tracker_1.componentStateTracker.emit('force-rerender');
                    // Reset component states
                    component_state_tracker_1.componentStateTracker.emit('reset-component-state', { all: true });
            ...
            validate: async () => {
                const recentEvents = component_state_tracker_1.componentStateTracker.getDesyncEvents(10);
        ...
        this.recoveryActions.set('PERFORMANCE_OPTIMIZATION', {
            ...
            execute: async () => {
                try {
                    ...
                    ui_performance_monitor_1.uiPerformanceMonitor.emit('optimize-memory');
                    ui_performance_monitor_1.uiPerformanceMonitor.emit('optimize-fps');
                    ui_performance_monitor_1.uiPerformanceMonitor.emit('optimize-layout');
    ...
    captureFunctionalityStatus() {
        return {
            claudeFunctionality: claude_functionality_validator_1.claudeFunctionalityValidator.getCriticalFailures(),
            streamingHealth: Array.from(sse_streaming_guardian_1.sseStreamingGuardian.getStreamingHealth().entries()),
            lastValidation: claude_functionality_validator_1.claudeFunctionalityValidator.getLastValidationTime()
        };
    }
    capturePerformanceMetrics() {
        return ui_performance_monitor_1.uiPerformanceMonitor.getCurrentMetrics();
    }
    calculateSnapshotConfidence() {
        ...
        const streamingHealth = sse_streaming_guardian_1.sseStreamingGuardian.getStreamingHealth();
        const performanceEvents = ui_performance_monitor_1.uiPerformanceMonitor.getPerformanceEvents(5);
        const stateEvents = component_state_tracker_1.componentStateTracker.getDesyncEvents(5);
        let confidence = 1.0;
    ...
    getElementSelector(element) {
        ...
        if (element.getAttribute('data-claude-action'))
            return `[data-claude-action="${element.getAttribute('data-claude-action')}"]`;
    ...
    async triggerRecovery(trigger, context) {
        ...
        try {
            ...
            if (neural_pattern_trainer_1.neuralPatternTrainer) {
                ...
                }, recoveryAttempt.status === 'SUCCESS' ? 'RECOVERY' : 'PARTIAL_REGRESSION', successRate, recoveryAttempt.duration);
            }
    ...
    selectRecoveryActions(trigger, context) {
        const actions = [];
    ...
    async restoreSnapshot(snapshotId) {
        ...
        try {
            ...
            domSnapshot.forEach((elementData) => {
    ...
    captureSystemState() {
        return {
            ...
            functionalityStatus: this.captureFunctionalityStatus(),
            performanceMetrics: this.capturePerformanceMetrics(),
            componentCount: component_state_tracker_1.componentStateTracker.getComponentStates().size,
            streamingConnections: sse_streaming_guardian_1.sseStreamingGuardian.getActiveConnections().length
        };
    }
    savePreReloadState() {
        const state = {
...
${Array.from(this.recoveryActions.values()).map(action => `- ${action.description} (${action.priority} priority, ${(action.successProbability * 100).toFixed(1)}% success rate)`).join('\n')}

Recent Recovery Attempts:
${recentAttempts.map(attempt => `${new Date(attempt.timestamp).toLocaleTimeString()} - ${attempt.trigger}: ${attempt.status} (${attempt.actions.length} actions, ${(attempt.duration / 1000).toFixed(2)}s)`).join('\n') || 'No recent attempts'}

System Health:
- Critical Functionality Issues: ${claude_functionality_validator_1.claudeFunctionalityValidator.getCriticalFailures().length}
- Active Streaming Connections: ${sse_streaming_guardian_1.sseStreamingGuardian.getActiveConnections().length}
- Component State Issues: ${component_state_tracker_1.componentStateTracker.getDesyncEvents(10).filter(e => e.severity === 'CRITICAL').length}
...
exports.AutomatedRecoverySystem = AutomatedRecoverySystem;
exports.automatedRecoverySystem = new AutomatedRecoverySystem();
//# sourceMappingURL=automated-recovery-system.js.map