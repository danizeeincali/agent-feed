...
exports.NetworkAntiPatternsDatabase = void 0;
class NetworkAntiPatternsDatabase {
    patterns = new Map();
    rules = new Map();
    detectedInstances = new Map();
    ...
    addCircuitBreakerBypassPattern() {
        this.addAntiPattern({
    ...
    analyzePatternTrends() {
        ...
        for (const [patternId, instances] of this.detectedInstances) {
            const recentInstances = instances.filter(instance => instance.timestamp > Date.now() - 3600000 // Last hour
            );
    ...
    calculateTrend(instances) {
        if (instances.length < 2) {
            return { direction: 'stable', magnitude: 0, confidence: 0 };
        }
        const timeWindow = 3600000; // 1 hour
        const recentCount = instances.filter(i => i.timestamp > Date.now() - timeWindow).length;
        const previousCount = instances.filter(i => i.timestamp > Date.now() - (timeWindow * 2) &&
            i.timestamp <= Date.now() - timeWindow).length;
        if (previousCount === 0) {
    ...
    // Public API
    detectAntiPattern(metrics, context) {
    ...
    recordDetection(patternId, metrics, context) {
        const instances = this.detectedInstances.get(patternId) || [];
        instances.push({
            timestamp: Date.now(),
            metrics,
    ...
    generatePreventionReport(patternIds) {
        ...
        const report = {
            ...
            prevention: {
                designPrinciples: new Set(),
                testingStrategies: new Set(),
                monitoringRequirements: new Set(),
                tddApproaches: new Set()
            },
            recommendations: []
        };
        ...
        return {
            ...
            recommendations: report.recommendations.sort((a, b) => (b.priority === 'high' ? 3 : b.priority === 'medium' ? 2 : 1) -
    ...
    validateDetection(patternId, instanceId, isValid) {
        const instances = this.detectedInstances.get(patternId) || [];
...
exports.NetworkAntiPatternsDatabase = NetworkAntiPatternsDatabase;
// Auto-initialize if in browser environment
if (typeof window !== 'undefined') {
    window.NLD_AntiPatternsDB = new NetworkAntiPatternsDatabase();
}
//# sourceMappingURL=network-anti-patterns-database.js.map