...
exports.FrontendBackendCommunicationMismatchDetector = void 0;
const events_1 = require("events");
const fs_1 = require("fs");
const path_1 = require("path");
class FrontendBackendCommunicationMismatchDetector extends events_1.EventEmitter {
    mismatches = [];
    networkTraces = [];
    dataDir;
    mismatchesFile;
    ...
    constructor() {
        ...
        this.mismatchesFile = (0, path_1.join)(this.dataDir, 'communication-mismatches.json');
        this.tracesFile = (0, path_1.join)(this.dataDir, 'network-traces.json');
    ...
    loadExistingData() {
        ...
        if ((0, fs_1.existsSync)(this.mismatchesFile)) {
            try {
                const data = (0, fs_1.readFileSync)(this.mismatchesFile, 'utf8');
        ...
        if ((0, fs_1.existsSync)(this.tracesFile)) {
            try {
                const data = (0, fs_1.readFileSync)(this.tracesFile, 'utf8');
    ...
    captureCommunicationMismatch(data) {
        const mismatch = {
    ...
    calculateSeverity(type, impact) {
        if (type === 'connection_failure' || impact.includes('complete'))
            return 'critical';
        if (type === 'protocol_mismatch' || type === 'auth_mismatch')
            return 'high';
        if (type === 'endpoint_mismatch' || type === 'message_format_mismatch')
            return 'medium';
        return 'low';
    }
    generateResolutionStrategy(type, frontend, backend) {
        const strategies = {
    ...
    validateEndpointCompatibility(frontendConfig, backendConfig) {
        const isCompatible = frontendConfig.protocol === backendConfig.protocol &&
    ...
    isMessageFormatCompatible(frontendFormat, backendFormat) {
    ...
    calculateConnectionSuccessRate() {
        if (this.networkTraces.length === 0)
            return 0;
        const successfulConnections = this.networkTraces.filter(t => t.type === 'response' && (t.status === 200 || t.status === 101) // 101 for WebSocket upgrade
        ).length;
    ...
    exportToNeuralTraining() {
        ...
        const exportPath = (0, path_1.join)(this.dataDir, 'neural-training-communication-mismatches.json');
    ...
    generateMismatchReport() {
        ...
        const reportPath = (0, path_1.join)(this.dataDir, `communication-mismatch-report-${Date.now()}.json`);
    ...
    persistData() {
        try {
            (0, fs_1.writeFileSync)(this.mismatchesFile, JSON.stringify(this.mismatches, null, 2));
            (0, fs_1.writeFileSync)(this.tracesFile, JSON.stringify(this.networkTraces, null, 2));
        }
        catch (error) {
...
exports.FrontendBackendCommunicationMismatchDetector = FrontendBackendCommunicationMismatchDetector;
//# sourceMappingURL=frontend-backend-communication-mismatch-detector.js.map