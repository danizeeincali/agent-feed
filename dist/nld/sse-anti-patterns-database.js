...
class SSEAntiPatternsDatabase {
     ...
     */
    findPatternsBySymptoms(symptoms) {
        const lowerSymptoms = symptoms.map(s => s.toLowerCase());
        return this.antiPatterns.filter(pattern => pattern.symptoms.some(symptom => lowerSymptoms.some(searchSymptom => symptom.toLowerCase().includes(searchSymptom))));
    }
    /**
     * Get prevention strategies for specific failure scenario
     */
    getPreventionStrategies(scenario) {
    ...
    generateTDDImplementationGuide() {
        return {
            testCategories: {
...
//# sourceMappingURL=sse-anti-patterns-database.js.map