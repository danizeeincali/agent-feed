...
exports.sseEndpointMismatchDetector = exports.SSEEndpointMismatchDetector = void 0;
const fs_1 = require("fs");
const path_1 = require("path");
/**
 * Main pattern detector for SSE endpoint mismatches
 */
class SSEEndpointMismatchDetector {
    patterns = new Map();
    patternStoragePath;
    constructor(storagePath = '/workspaces/agent-feed/src/nld/patterns') {
     ...
     */
    async detectEndpointMismatches(projectPath = '/workspaces/agent-feed') {
        ...
        const inconsistencies = this.analyzeVersioningInconsistencies(sseEndpoints, restEndpoints, backendRoutes);
        // Create pattern if inconsistencies found
        if (inconsistencies.length > 0) {
            const pattern = this.createEndpointMismatchPattern(sseEndpoints, restEndpoints, backendRoutes, inconsistencies);
     ...
     */
    async extractSSEEndpoints(projectPath) {
        const endpoints = [];
     ...
     */
    async extractRESTEndpoints(projectPath) {
        const endpoints = [];
     ...
     */
    async extractBackendRoutes(projectPath) {
        const routes = [];
     ...
     */
    analyzeVersioningInconsistencies(sseEndpoints, restEndpoints, backendRoutes) {
        ...
        sseEndpoints.forEach(sseEndpoint => {
            if (!sseEndpoint.version) {
                ...
                const hasVersioned = backendRoutes.some(route => route.path === versionedEndpoint ||
     ...
     */
    createEndpointMismatchPattern(sseEndpoints, restEndpoints, backendRoutes, inconsistencies) {
        ...
        const neuralFeatures = this.extractNeuralTrainingFeatures(sseEndpoints, restEndpoints, inconsistencies);
        return {
            id: patternId,
     ...
     */
    calculateFailureProbability(inconsistencies) {
        if (inconsistencies.length === 0)
            return 0.0;
     ...
     */
    calculateImpactScore(endpoints, inconsistencies) {
     ...
     */
    calculateDetectionConfidence(inconsistencies) {
        ...
        if (pathMismatches > 0 && protocolMismatches > 0)
            return 0.95;
        if (pathMismatches > 0)
            return 0.85;
        if (protocolMismatches > 0)
            return 0.75;
        return 0.6;
    }
    /**
     ...
     */
    calculateSeverity(inconsistencies) {
        if (inconsistencies.length === 0)
            return 'low';
     ...
     */
    assessBusinessImpact(inconsistencies) {
        const impacts = [];
        const hasSSEFailures = inconsistencies.some(inc => inc.frontendPath.includes('terminal/stream') ||
     ...
     * Generate prevention strategies
     */
    generatePreventionStrategies(inconsistencies) {
        const strategies = [];
     ...
     */
    extractNeuralTrainingFeatures(sseEndpoints, restEndpoints, inconsistencies) {
        const features = [];
        // Detection features
        features.push({
     ...
     */
    generateMockSSEAttempts(endpoints) {
     ...
     */
    generateMockRESTRequests(endpoints) {
        return endpoints.map(endpoint => ({
    ...
    loadExistingPatterns() {
        try {
            const patternsFile = (0, path_1.join)(this.patternStoragePath, 'sse-endpoint-mismatch-patterns.json');
            if ((0, fs_1.existsSync)(patternsFile)) {
                const data = (0, fs_1.readFileSync)(patternsFile, 'utf-8');
    ...
    savePattern(pattern) {
        try {
            ...
            (0, fs_1.writeFileSync)(patternsFile, JSON.stringify(allPatterns, null, 2));
     ...
     */
    generateGlobalRecommendations(patterns) {
        ...
        const sseFailures = patterns.some(p => p.versioningInconsistencies.some(inc => inc.frontendPath.includes('stream')));
        if (sseFailures) {
...
exports.SSEEndpointMismatchDetector = SSEEndpointMismatchDetector;
// Export singleton instance
exports.sseEndpointMismatchDetector = new SSEEndpointMismatchDetector();
//# sourceMappingURL=sse-endpoint-mismatch-pattern-detector.js.map