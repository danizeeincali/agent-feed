...
exports.MixedAPIVersioningNLDDeployment = void 0;
const mixed_api_versioning_anti_pattern_detector_1 = require("./mixed-api-versioning-anti-pattern-detector");
const mixed_api_versioning_prevention_strategies_1 = require("./mixed-api-versioning-prevention-strategies");
const mixed_api_versioning_neural_training_export_1 = require("./mixed-api-versioning-neural-training-export");
 ...
 */
class MixedAPIVersioningNLDDeployment {
    detector;
    ...
    constructor() {
        ...
        this.preventionStrategies = new mixed_api_versioning_prevention_strategies_1.MixedAPIVersioningPreventionStrategies();
        this.neuralExport = new mixed_api_versioning_neural_training_export_1.MixedAPIVersioningNeuralTrainingExport();
        this.deploymentPath = path.join(process.cwd(), 'src/nld/deployment-reports');
    }
    /**
     * Complete NLD deployment process
     */
    async deployNLDSystem() {
     ...
     */
    async generateComprehensiveTDDTests(detectedPatterns) {
        const testCasesPath = path.join(process.cwd(), 'src/nld/patterns/test-cases');
        await fs_1.promises.mkdir(testCasesPath, { recursive: true });
        ...
        for (const [filename, content] of Object.entries(testCases)) {
            await fs_1.promises.writeFile(path.join(testCasesPath, filename), content, 'utf-8');
        }
     ...
     */
    generateMixedVersioningTests(patterns) {
        const specificEndpoints = patterns.flatMap(p => p.endpointPairs.map((ep) => ({
...
describe('Mixed API Versioning Prevention - Real Patterns', () => {
  ...
  test('should prevent ${patterns[0]?.impactAssessment.userWorkflowBroken ? 'critical' : 'partial'} workflow failures', async () => {
     ...
     */
    generateEndpointConsistencyTests() {
...
describe('Complete User Workflow - No Mixed Versioning', () => {
  test('should complete Claude instance lifecycle without API inconsistencies', async () => {
     ...
     */
    generateNeuralPatternTests(patterns) {
...
describe('Neural Pattern Detection - Mixed API Versioning', () => {
  ...
  test('should learn from real failure patterns', async () => {
    ...
    async generateDeploymentReport(data) {
        await fs_1.promises.mkdir(this.deploymentPath, { recursive: true });
        const report = {
            ...
            pattern_analysis: {
                ...
                critical_impact_patterns: data.detectedPatterns.filter((p) => p.impactAssessment.userWorkflowBroken).length,
                silent_failure_risk: data.detectedPatterns.filter((p) => p.impactAssessment.silentFailures).length
            },
...
exports.MixedAPIVersioningNLDDeployment = MixedAPIVersioningNLDDeployment;
exports.default = MixedAPIVersioningNLDDeployment;
//# sourceMappingURL=deploy-mixed-api-versioning-nld.js.map