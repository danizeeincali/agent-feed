...
exports.sseIntegrationGapMonitor = exports.SSEIntegrationGapMonitor = void 0;
const events_1 = require("events");
const fs_1 = require("fs");
const path_1 = require("path");
const child_process_1 = require("child_process");
class SSEIntegrationGapMonitor extends events_1.EventEmitter {
    gapPatterns = [];
    codebaseAnalysis = new Map();
    ...
    findSSEHelperFunctions() {
        try {
            ...
            const files = (0, child_process_1.execSync)(searchCommand, { encoding: 'utf8' }).trim().split('\n').filter(f => f);
            const helpers = [];
            for (const file of files) {
                try {
                    const content = (0, child_process_1.execSync)(`cat "${file}"`, { encoding: 'utf8' });
    ...
    checkIntegrationGaps(helpers, endpoints) {
    ...
    persistGapPattern(pattern) {
        const patternFile = (0, path_1.join)(this.storageDir, 'integration-gap-patterns.jsonl');
        require('fs').appendFileSync(patternFile, JSON.stringify(pattern) + '\n', { flag: 'a' });
    }
    /**
     * Export TDD prevention strategies based on detected gaps
     */
    exportTDDPreventionStrategies() {
        const strategies = {
        ...
        const exportFile = (0, path_1.join)(this.storageDir, 'tdd-prevention-strategies.json');
        (0, fs_1.writeFileSync)(exportFile, JSON.stringify(strategies, null, 2));
...
exports.SSEIntegrationGapMonitor = SSEIntegrationGapMonitor;
// Export singleton instance
exports.sseIntegrationGapMonitor = new SSEIntegrationGapMonitor();
//# sourceMappingURL=sse-integration-gap-monitor.js.map