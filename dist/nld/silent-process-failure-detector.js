...
exports.silentProcessDetector = exports.SilentProcessFailureDetector = void 0;
const events_1 = require("events");
class SilentProcessFailureDetector extends events_1.EventEmitter {
    monitoredProcesses = new Map();
    detectedPatterns = new Map();
    alertHistory = [];
    isMonitoring = false;
    // Configuration thresholds
    SILENT_DETECTION_THRESHOLD = 8000; // 8 seconds
    TTY_CHECK_COMMANDS = ['vi', 'nano', 'less', 'more', 'ssh', 'sudo'];
    AUTH_DETECTION_KEYWORDS = ['password:', 'passphrase:', 'enter password', 'authentication required', 'login:'];
    PERMISSION_ERROR_KEYWORDS = ['permission denied', 'access denied', 'operation not permitted', 'insufficient privileges'];
    constructor() {
        super();
     ...
     */
    recordOutput(instanceId, outputType, data) {
    ...
    recordProcessEnd(instanceId, exitCode) {
        ...
        if (exitCode && this.isPermissionExitCode(exitCode)) {
            this.triggerAlert(instanceId, 'permission_denied', 'critical', `Process terminated with permission error (exit code: ${exitCode})`, 'WORKING_DIRECTORY_PERMISSIONS');
        }
    }
    /**
     ...
     */
    performSilentProcessChecks() {
        const now = new Date();
        for (const [instanceId, metrics] of this.monitoredProcesses) {
            if (metrics.processStatus === 'terminated')
                continue;
     ...
     */
    detectSilentProcessPattern(instanceId) {
        const metrics = this.monitoredProcesses.get(instanceId);
        if (!metrics)
            return;
        ...
        if (detectedPatterns.length > 0) {
            ...
            this.triggerAlert(instanceId, 'silent_process_detected', pattern.severity, `Silent process failure detected: ${pattern.patternName}`, primaryPattern);
        }
    }
    /**
     * Check if metrics match a specific pattern
     */
    matchesPattern(metrics, pattern) {
        const criteria = pattern.detectionCriteria;
        // Check PID existence
        if (criteria.processPidExists && !metrics.processId)
            return false;
        // Check silent duration
        if (criteria.stdoutSilentDuration && metrics.silentDuration < criteria.stdoutSilentDuration)
            return false;
        // Check process still running (if required)
        if (criteria.processStillRunning && metrics.processStatus === 'terminated')
            return false;
     ...
     */
    requiresAuthentication(command) {
     ...
     */
    performEnvironmentChecks(instanceId, workingDirectory) {
        const metrics = this.monitoredProcesses.get(instanceId);
        if (!metrics)
            return;
        ...
        metrics.environmentChecks.missingEnvironmentVars = criticalEnvVars.filter(varName => !process.env[varName]);
        this.monitoredProcesses.set(instanceId, metrics);
     ...
     */
    startProcessMonitoring(instanceId) {
     ...
     */
    analyzeErrorOutput(instanceId, errorData) {
        const errorLower = errorData.toLowerCase();
        // Check for permission errors
        if (this.PERMISSION_ERROR_KEYWORDS.some(keyword => errorLower.includes(keyword))) {
            this.triggerAlert(instanceId, 'permission_denied', 'high', `Permission error detected: ${errorData.trim()}`, 'WORKING_DIRECTORY_PERMISSIONS');
        }
        // Check for authentication prompts
        if (this.AUTH_DETECTION_KEYWORDS.some(keyword => errorLower.includes(keyword))) {
            this.triggerAlert(instanceId, 'auth_prompt_detected', 'medium', `Authentication prompt detected: ${errorData.trim()}`, 'AUTH_PROMPT_INVISIBLE');
        }
    }
    /**
     * Check if exit code indicates permission error
     */
    isPermissionExitCode(exitCode) {
     ...
     */
    triggerAlert(instanceId, alertType, severity, message, detectedPattern) {
        const metrics = this.monitoredProcesses.get(instanceId);
     ...
     */
    clearSilentProcessAlert(instanceId) {
...
exports.SilentProcessFailureDetector = SilentProcessFailureDetector;
// Export singleton instance
exports.silentProcessDetector = new SilentProcessFailureDetector();
//# sourceMappingURL=silent-process-failure-detector.js.map