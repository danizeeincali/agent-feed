...
exports.NLDNetworkDeploymentValidator = void 0;
class NLDNetworkDeploymentValidator {
    components = new Map();
    healthHistory = [];
    deploymentId;
    version = '1.0.0';
    ...
    initializeComponents() {
        ...
        try {
            this.components.set('detector', window.NLD_NetworkDetector);
            this.components.set('monitor', window.NLD_NetworkMonitor);
            this.components.set('cors_timeout', window.NLD_CORSTimeoutDetector);
            this.components.set('api_analyzer', window.NLD_APIAnalyzer);
            this.components.set('communication', window.NLD_CommunicationAnalyzer);
            this.components.set('neural_exporter', window.NLD_NeuralExporter);
            this.components.set('anti_patterns', window.NLD_AntiPatternsDB);
            this.components.set('tdd_strategies', window.NLD_TDDStrategies);
        }
        catch (error) {
    ...
    async validateCORSTimeoutDetector() {
    ...
    async validateCommunicationAnalyzer() {
    ...
    async assessNeuralTraining() {
        try {
            ...
            if (recentExports && metrics.averageConfidence > 0.7)
                return 'active';
    ...
    calculateOverallScore(components, integration, performance) {
        ...
        if (integration.dataFlow === 'healthy')
            integrationScore += 10;
        else if (integration.dataFlow === 'partial')
            integrationScore += 5;
        if (integration.neuralTraining === 'active')
            integrationScore += 10;
        else if (integration.neuralTraining === 'limited')
            integrationScore += 5;
        integrationScore += integration.tddCoverage * 5;
        integrationScore += integration.patternAccuracy * 5;
        // Performance score (30%)
        let performanceScore = 30;
        if (performance.averageDetectionTime > 1000)
            performanceScore -= 5;
        if (performance.systemResponseTime > 500)
            performanceScore -= 5;
        if (performance.memoryFootprint > 50 * 1024 * 1024)
            performanceScore -= 5; // 50MB
        if (performance.throughput < 5)
            performanceScore -= 5;
        return Math.max(0, Math.min(100, componentScore + integrationScore + performanceScore));
    }
    determineOverallStatus(score, components) {
        ...
        if (failedComponents > components.length / 2)
            return 'failed';
        if (score < 50 || failedComponents > 0)
            return 'critical';
        if (score < 75 || criticalComponents > 1)
            return 'degraded';
        return 'healthy';
    }
    ...
    createFailedComponent(id, name, reason) {
        return {
            componentId: id,
            name,
            status: 'failed',
    ...
    identifyDetectorIssues(patterns, metrics) {
        const issues = [];
    ...
    identifyMonitorIssues(metrics, alerts) {
        const issues = [];
        if (alerts.length > 50) {
            issues.push({
                severity: 'medium',
    ...
    identifyCORSTimeoutIssues(corsPatterns, timeoutPatterns, metrics) {
        const issues = [];
    ...
    identifyAPIAnalyzerIssues(mismatches, metrics, recommendations) {
        const issues = [];
    ...
    identifyCommunicationIssues(patterns, health, antiPatterns) {
        const issues = [];
    ...
    identifyNeuralExporterIssues(exportHistory, metrics) {
        const issues = [];
    ...
    identifyAntiPatternsDBIssues(patterns, statistics, report) {
        const issues = [];
    ...
    identifyTDDStrategiesIssues(strategies, metrics) {
        const issues = [];
    ...
    generateSystemRecommendations(components, integration) {
    ...
    generateRecommendations(systemHealth, patternAnalysis, neuralTraining, tddIntegration) {
        const immediate = [];
        const shortTerm = [];
        const longTerm = [];
        // Immediate actions
    ...
    identifyImplementationGaps(strategies, preventionReport) {
        const gaps = [];
        ...
        if (!strategies.some((s) => s.id.includes('CORS'))) {
            gaps.push('Missing CORS-specific TDD strategies');
        }
        if (!strategies.some((s) => s.id.includes('TIMEOUT'))) {
    ...
    async quickHealthCheck() {
        ...
        const criticalIssues = health.components.reduce((count, c) => count + c.issues.filter(i => i.severity === 'critical').length, 0);
        return {
...
exports.NLDNetworkDeploymentValidator = NLDNetworkDeploymentValidator;
// Auto-initialize if in browser environment
if (typeof window !== 'undefined') {
    window.NLD_DeploymentValidator = new NLDNetworkDeploymentValidator();
}
//# sourceMappingURL=nld-network-deployment-validator.js.map