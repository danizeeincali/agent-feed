...
exports.ClaudeFlowIntegration = void 0;
...
const neural_connection_trainer_1 = require("./neural-connection-trainer");
const adaptive_connection_manager_1 = require("./adaptive-connection-manager");
class ClaudeFlowIntegration extends events_1.EventEmitter {
    failureDetector;
    learningDatabase;
    neuralTrainer;
    adaptiveManager;
    ...
    initializeComponents() {
        ...
        this.neuralTrainer = new neural_connection_trainer_1.NeuralConnectionTrainer({
            batchSize: 50,
            learningRate: 0.001,
            epochs: 100,
            validationSplit: 0.2,
    ...
    setupIntegration() {
        ...
        this.failureDetector.on('patternDetected', async (data) => {
            await this.learningDatabase.storeFailurePattern(data.context, data.pattern, { source: 'failure_detector' });
            // Store in claude-flow memory
            await this.storeInClaudeFlowMemory(`nld/patterns/${data.pattern.id}`, data.pattern);
        });
     ...
     */
    async storeInClaudeFlowMemory(key, data, ttl) {
        try {
     ...
     */
    async retrieveFromClaudeFlowMemory(key) {
        try {
            // Use MCP memory system
     ...
     */
    async trainClaudeFlowNeuralPatterns(trainingData) {
        ...
        try {
            ...
            await this.storeInClaudeFlowMemory(`nld/training/${Date.now()}`, { request, result: 'pending' });
        }
        catch (error) {
     ...
     */
    async orchestrateNeuralTraining(data) {
        if (!this.config.taskOrchestrationEnabled)
            return;
        try {
            const task = {
     ...
     */
    trackPerformanceMetrics(type, data) {
        ...
        this.storeInClaudeFlowMemory(`nld/metrics/${type}/${Date.now()}`, metrics, 3600000 // 1 hour TTL for metrics
        );
...
exports.ClaudeFlowIntegration = ClaudeFlowIntegration;
//# sourceMappingURL=claude-flow-integration.js.map