...
class SecurityManager extends events_1.EventEmitter {
    ...
    calculateRiskScore(eventData) {
        ...
        if (eventData.outcome === 'failure')
            score += 20;
        if (eventData.outcome === 'blocked')
            score += 15;
        // Additional risk factors
        if (eventData.ip && this.isKnownThreatIP(eventData.ip))
            score += 25;
        if (eventData.user && this.isHighPrivilegeUser(eventData.user))
            score += 15;
    ...
    checkSecurityRules(securityEvent) {
        for (const rule of this.securityRules.values()) {
            if (!rule.enabled || rule.type !== securityEvent.type)
                continue;
    ...
    evaluateSecurityRule(rule, event) {
    ...
    checkFailedLoginAttempts(event, rule) {
        ...
        const recentFailures = Array.from(this.securityEvents.values()).filter(e => e.type === 'authentication' &&
            e.action === 'login' &&
            e.outcome === 'failure' &&
            e.ip === event.ip &&
    ...
    checkUnusualDataAccess(event, rule) {
    ...
    checkBruteForceAttack(event, rule) {
        const timeWindow = (rule.timeWindow || 1) * 60 * 1000; // Convert to milliseconds
        const threshold = rule.threshold || 100;
        const cutoffTime = Date.now() - timeWindow;
        const recentRequests = Array.from(this.securityEvents.values()).filter(e => e.ip === event.ip &&
            e.timestamp >= cutoffTime);
    ...
    createSecurityAlert(rule, event) {
    ...
    analyzeSecurityEvents() {
        const recentEvents = Array.from(this.securityEvents.values()).filter(event => Date.now() - event.timestamp < 3600000 // Last hour
        );
        // Pattern analysis
    ...
    detectThreats() {
        ...
        for (const threat of this.threatDetections.values()) {
            if (!threat.mitigated) {
                ...
                const recentEvents = Array.from(this.securityEvents.values()).filter(event => Date.now() - event.timestamp < 300000 // Last 5 minutes
                );
    ...
    filterEventsByThreat(events, threat) {
    ...
    updateSecurityMetrics() {
        const recentEvents = Array.from(this.securityEvents.values()).filter(event => Date.now() - event.timestamp < 86400000 // Last 24 hours
        );
        const metrics = {
    ...
    getSecurityMetrics() {
        return {
            totalEvents: this.securityEvents.size,
            eventsLast24h: Array.from(this.securityEvents.values()).filter(event => Date.now() - event.timestamp < 86400000).length,
            threatsDetected: this.threatDetections.size,
            threatsBlocked: Array.from(this.threatDetections.values()).filter(t => t.mitigated).length,
            complianceScore: this.calculateComplianceScore(),
            riskScore: this.calculateOverallRiskScore(),
