/**
 * TDD London School - SSE Output Chunking Contract Tests (FIXED)
 * 
...
describe('TDD London School - SSE Output Chunking Contracts', () => {
  ...
  describe('CRITICAL: SSE Only Sends NEW Output (No Buffer Replay)', () => {
    test('should track output position per Claude instance', () => {
      ...
      const positionTracker = {
        ...
        updatePosition(instanceId, newPos) {
          this.positions.set(instanceId, newPos);
          return newPos;
        },
        
        getNewContentSince(instanceId, content) {
          const lastPos = this.getPosition(instanceId);
          const newContent = content.substring(lastPos);
          this.updatePosition(instanceId, content.length);
          return newContent;
        }
      };
    ...
    test('should prevent full buffer replay on reconnection', () => {
      // Reconnection behavior contract
      const instanceId = 'claude-123';
  ...
  describe('CRITICAL: Output Position Tracking Per Claude Instance', () => {
    test('should maintain separate position cursors per instance', () => {
      // Multi-instance position isolation
      ...
      const multiInstanceTracker = {
        ...
        track(instanceId, content) {
          const currentPos = this.positions.get(instanceId) || 0;
          const newContent = content.substring(currentPos);
          this.positions.set(instanceId, content.length);
          return newContent;
        },
  ...
  describe('CRITICAL: Message Deduplication in Frontend', () => {
    test('should deduplicate messages based on content hash', () => {
      // Frontend deduplication contract
      const messageDeduplicator = {
        seenHashes: new Set(),
        
        isDuplicate(message) {
          const hash = this.createHash(message);
        ...
        processMessage(message) {
          ...
          return { processed: true, hash: this.createHash(message) };
        }
      };
    ...
    test('should handle rapid "hello" inputs without repetition', () => {
      // Rapid input deduplication
      const rapidInputHandler = {
        lastInputTime: new Map(),
        inputDebounceMs: 100,
        
        shouldProcess(instanceId, input) {
          const now = Date.now();
          const lastTime = this.lastInputTime.get(instanceId) || 0;
          const timeSinceLastInput = now - lastTime;
          
          if (timeSinceLastInput < this.inputDebounceMs && input === 'hello') {
            return { process: false, reason: 'debounced', waitTime: this.inputDebounceMs - timeSinceLastInput };
          }
          
          this.lastInputTime.set(instanceId, now);
          return { process: true };
        }
      };
      ...
      const connectionStateManager = {
        ...
        setState(instanceId, state) {
          this.states.set(instanceId, {
            state,
            timestamp: Date.now(),
            transitions: (this.states.get(instanceId)?.transitions || 0) + 1
          });
        },
        
        getState(instanceId) {
          return this.states.get(instanceId)?.state || 'disconnected';
        },
        ...
        canSendMessages(instanceId) {
          return this.isReady(instanceId) && this.hasActiveConnections(instanceId);
        },
        
        hasActiveConnections(instanceId) {
    ...
    test('should manage growing output buffers efficiently', () => {
      ...
      const bufferManager = {
        ...
        appendToBuffer(instanceId, newContent) {
          const current = this.buffers.get(instanceId) || '';
      ...
      // Large append that triggers trimming
      ...
      // Append more that exceeds limit 
