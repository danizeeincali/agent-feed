...
test.describe('Agent Slug Navigation', () => {
  ...
  test('10. Should load agent data from API using slug', async ({ page, request }) => {
    ...
    const slugApiCall = apiCalls.find(url =>
      url.includes(testAgent.slug) || url.includes('/api/agents')
    );
    ...
    const apiResponse = await request.get(`${BASE_URL}/api/agents/${testAgent.slug}`);
    expect(apiResponse.ok()).toBeTruthy();

    const agentData = await apiResponse.json();
    const agent = agentData.data || agentData;
    expect(agent.slug).toBe(testAgent.slug);
