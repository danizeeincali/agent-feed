...
async function validateRaceConditionFix() {
  ...
  try {
    ...
    try {
      await Promise.race([
        page.waitForSelector('[data-testid="rendered-page"]', { timeout: 5000 }),
        page.waitForSelector('.prose', { timeout: 5000 }),
        page.waitForSelector('h2:has-text("Personal Todos Dashboard")', { timeout: 5000 }),
        page.waitForFunction(() => {
          const text = document.body.innerText;
          return text.includes('Personal Todos Dashboard') || text.includes('No pages yet');
        }, { timeout: 5000 })
      ]);
      console.log('✅ Content indicators found');
    } catch (e) {
      console.log('⚠️ No specific content indicators found, continuing...');
    }
    ...
    console.log(`React errors found: ${results.reactErrors.length}`);
    results.reactErrors.forEach(error => console.log(`  🔴 ${error}`));
    
    // Step 6: API validation
    console.log('\n🌐 API call summary...');
    console.log(`Successful API calls: ${results.apiCalls.successful.length}`);
    console.log(`Failed API calls: ${results.apiCalls.failed.length}`);
    
    // Check for specific agent pages API call
    const agentPagesApiCall = results.apiCalls.successful.find(call => 
      call.url.includes('/agents/personal-todos-agent/pages')
    );
    
    if (agentPagesApiCall) {
      console.log('✅ Agent pages API call successful');
    } else {
      console.log('❌ Agent pages API call not found or failed');
    }
