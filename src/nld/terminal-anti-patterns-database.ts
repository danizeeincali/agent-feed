...
export class TerminalAntiPatternsDatabase {
   ...
   */
  private addAntiPattern(pattern: Omit<AntiPattern, 'id'> & { id: string }): void {
    this.antiPatterns.set(pattern.id, pattern as AntiPattern);
   ...
   */
  public detectAntiPatterns(output: string, context?: any): Array<{
    pattern: AntiPattern;
    confidence: number;
    matchedRules: string[];
  }> {
   ...
   */
  private evaluatePattern(pattern: AntiPattern, output: string, context?: any): {
    confidence: number;
    matchedRules: string[];
  } {
    let confidence = 0;
    ...
    const patternMatches = pattern.detectionRules.patterns.filter(p => 
   ...
   */
  private evaluateContextRule(rule: string, output: string, context: any): boolean {
    switch (rule) {
      case 'repeated_identical_responses':
        return context.responseHistory && 
               context.responseHistory.length > 1 &&
               context.responseHistory.every((r: string) => r === context.responseHistory[0]);
      ...
      case 'missing_error_responses':
        return context.commandHistory && 
               context.commandHistory.some((cmd: string) => cmd.includes('invalid')) &&
   ...
   */
  private calculatePatternSpecificConfidence(pattern: AntiPattern, output: string, context?: any): number {
    let boost = 0;

    switch (pattern.id) {
      case 'mock-terminal-responses':
        if (output.includes('HTTP/SSE') && output.includes('WebSocket')) boost += 0.2;
        if (output.includes('[timestamp]') || /\[\d{2}:\d{2}:\d{2}\]/.test(output)) boost += 0.1;
        break;
      
      case 'hardcoded-command-responses':
        if (output.includes('total 8') && output.includes('claude claude')) boost += 0.15;
        if (output.includes('System uptime:') && output.includes('days')) boost += 0.1;
        break;
      
      case 'stdout-pipe-disconnection':
        if (context?.processRunning && (!output || output.trim().length < 5)) boost += 0.3;
        break;
      
      case 'sse-broadcast-failure':
        if (context?.eventsGenerated && context?.eventsReceived < context.eventsGenerated * 0.5) boost += 0.2;
        break;
      
      case 'working-directory-mismatch':
        if (context?.workingDirectory && !context.workingDirectory.startsWith('/workspaces/agent-feed')) boost += 0.2;
        break;
    }
   ...
   */
  private containsProcessSpecificOutput(output: string): boolean {
   ...
   */
  public getAntiPattern(id: string): AntiPattern | undefined {
   ...
   */
  public getAntiPatternsByCategory(category: AntiPattern['category']): AntiPattern[] {
  ...
  public getStatistics(): {
    ...
    mostFrequent: Array<{ id: string; frequency: number; pattern: AntiPattern }>;
   ...
   */
  public exportForNeuralTraining(): string {
