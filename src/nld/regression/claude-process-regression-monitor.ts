...
export class ClaudeProcessRegressionMonitor {
  private patterns: Map<string, ClaudeProcessRegressionPattern> = new Map();
  private events: ClaudeProcessEvent[] = [];
  private alerts: RegressionAlert[] = [];
  private isMonitoring: boolean = false;
  private detectionInterval?: NodeJS.Timeout;
  private baselineConfiguration: any = null;
   ...
   */
  private initializeCriticalPatterns(): void {
    const criticalPatterns: ClaudeProcessRegressionPattern[] = [
      {
   ...
   */
  public recordEvent(event: ClaudeProcessEvent): void {
   ...
   */
  private checkEventForRegressions(event: ClaudeProcessEvent): void {
   ...
   */
  private performPatternDetection(): void {
    const startTime = performance.now();
    ...
    for (const pattern of this.patterns.values()) {
      ...
      for (const event of recentEvents) {
        if (this.isPatternTriggered(pattern, event)) {
          ...
          const recentAlertExists = this.alerts.some(alert => 
            alert.pattern.id === pattern.id &&
   ...
   */
  private isPatternTriggered(pattern: ClaudeProcessRegressionPattern, event: ClaudeProcessEvent): boolean {
   ...
   */
  private generateAlert(pattern: ClaudeProcessRegressionPattern, event: ClaudeProcessEvent): RegressionAlert {
   ...
   */
  private calculateConfidence(pattern: ClaudeProcessRegressionPattern, event: ClaudeProcessEvent): number {
    let confidence = 0.5; // Base confidence

    // Higher confidence for critical patterns
    if (pattern.severity === 'CRITICAL') confidence += 0.3;
    if (pattern.severity === 'HIGH') confidence += 0.2;

    // Higher confidence for spawn and error events
    if (event.eventType === 'ERROR') confidence += 0.2;
    if (event.eventType === 'SPAWN') confidence += 0.1;
   ...
   */
  private triggerImmediateResponse(alert: RegressionAlert): void {
   ...
   */
  private executeRecoveryAction(alert: RegressionAlert): void {
   ...
   */
  private fixPrintFlagRegression(alert: RegressionAlert): void {
   ...
   */
  private fixMockClaudeRegression(alert: RegressionAlert): void {
   ...
   */
  private fixAuthenticationRegression(alert: RegressionAlert): void {
   ...
   */
  private fixWorkingDirectoryRegression(alert: RegressionAlert): void {
   ...
   */
  private fixProcessSpawningRegression(alert: RegressionAlert): void {
   ...
   */
  private notifyDashboard(alert: RegressionAlert): void {
...
export const claudeProcessRegressionMonitor = new ClaudeProcessRegressionMonitor();

// Auto-start monitoring
claudeProcessRegressionMonitor.startMonitoring();
