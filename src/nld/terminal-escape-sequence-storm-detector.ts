...
export class TerminalEscapeSequenceStormDetector extends EventEmitter {
  private detectedPatterns: EscapeSequencePattern[] = [];
  private stormPatterns: TerminalStormPattern[] = [];
  private patternStorage: string;
  private sequenceBuffer: Map<string, { count: number; firstSeen: number; lastSeen: number }> = new Map();
  private readonly stormThreshold = 50; // Repetitions to trigger storm detection
  ...
  constructor(storageDir: string) {
    super();
    this.patternStorage = join(storageDir, 'terminal-escape-sequence-storm-patterns.json');
   ...
   */
  private extractEscapeSequences(output: string): Array<{type: 'escape_sequence' | 'message', content: string}> {
    const chunks: Array<{type: 'escape_sequence' | 'message', content: string}> = [];
   ...
   */
  private trackSequenceRepetition(sequence: string, timestamp: number): void {
   ...
   */
  private trackMessageRepetition(message: string, timestamp: number): void {
    ...
    const matchedMessage = knownProblematicMessages.find(known => 
      message.includes(known) || known.includes(message.slice(0, 50))
    );
   ...
   * Check for storm patterns
   */
  private checkForStormPatterns(instanceId: string): void {
    const now = Date.now();
    const stormSequences: string[] = [];
    const stormMessages: string[] = [];
    ...
    if (stormSequences.length > 0 || stormMessages.length > 0) {
      this.recordStormPattern(stormSequences, stormMessages, totalRepetitions, 
        latestTime - earliestTime, instanceId);
    }
  }
   ...
   */
  private recordEscapeSequencePattern(key: string, data: any, instanceId: string): void {
    const content = key.startsWith('seq:') ? key.replace('seq:', '') : key.replace('msg:', '');
    const isSequence = key.startsWith('seq:');
    
    const pattern: EscapeSequencePattern = {
   ...
   */
  private recordStormPattern(sequences: string[], messages: string[], 
    totalRepetitions: number, durationMs: number, instanceId: string): void {
    
    const storm: TerminalStormPattern = {
   ...
   */
  private classifySequence(sequence: string): EscapeSequencePattern['sequenceType'] {
    return (this.knownSequences as any)[sequence] || 'unknown';
  }

  /**
   * Calculate pattern severity
   */
  private calculateSeverity(repetitionCount: number): EscapeSequencePattern['severity'] {
    if (repetitionCount >= 1000) return 'critical';
    if (repetitionCount >= 500) return 'high';
    if (repetitionCount >= 100) return 'medium';
    return 'low';
  }
   ...
   */
  private analyzeRootCause(content: string, repetitionCount: number): EscapeSequencePattern['rootCause'] {
   ...
   */
  private estimateProcessSpawns(repetitionCount: number): number {
   ...
   */
  private estimateSSEConnections(repetitionCount: number): number {
   ...
   */
  private estimateClickEvents(content: string, repetitionCount: number): number {
   ...
   * Classify storm type
   */
  private classifyStorm(sequences: string[], messages: string[]): TerminalStormPattern['stormType'] {
    if (sequences.length > 0 && messages.length > 0) return 'mixed_storm';
    if (sequences.length > 0) return 'escape_sequence_storm';
   ...
   */
  private calculateStormSeverity(totalRepetitions: number, durationMs: number): TerminalStormPattern['severity'] {
    const ratePerSecond = totalRepetitions / (durationMs / 1000);
    
    if (ratePerSecond >= 100 || totalRepetitions >= 5000) return 'critical';
    if (ratePerSecond >= 50 || totalRepetitions >= 2000) return 'high';
    if (ratePerSecond >= 20 || totalRepetitions >= 500) return 'medium';
    return 'low';
  }
   ...
   */
  private analyzeStormRootCause(sequences: string[], messages: string[], totalRepetitions: number): string {
   ...
   */
  private suggestPreventionStrategy(sequences: string[], messages: string[]): string {
    if (sequences.includes('[?25l') || sequences.includes('[?25h')) {
