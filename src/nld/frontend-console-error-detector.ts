...
export class FrontendConsoleErrorDetector extends EventEmitter {
  private errors: ConsoleError[] = [];
  private errorSignatures: Map<string, RefactoringErrorSignature> = new Map();
  private readonly dataDir: string;
  private readonly errorsFile: string;
  private readonly signaturesFile: string;
  ...
  constructor() {
    ...
    this.errorsFile = join(this.dataDir, 'frontend-console-errors.json');
    this.signaturesFile = join(this.dataDir, 'error-signatures.json');
  ...
  private loadExistingData(): void {
    ...
    if (existsSync(this.signaturesFile)) {
      try {
  ...
  private simulateRefactoringErrors(): void {
    const commonErrors = [
      {
        type: 'error' as const,
  ...
  public captureConsoleError(errorData: Partial<ConsoleError> & { component?: string; action?: string }): void {
  ...
  private calculateSeverity(message: string, type: string): 'low' | 'medium' | 'high' | 'critical' {
    if (type === 'error') {
  ...
  private categorizeError(message: string): 'refactoring' | 'network' | 'runtime' | 'syntax' | 'reference' {
  ...
  private updateErrorSignature(error: ConsoleError): void {
  ...
  private generateErrorSignature(error: ConsoleError): string {
  ...
  private persistErrors(): void {
    try {
      // Save errors
      writeFileSync(this.errorsFile, JSON.stringify(this.errors, null, 2));
      
      // Save signatures as object for JSON compatibility
      const signaturesObj = Object.fromEntries(this.errorSignatures);
      writeFileSync(this.signaturesFile, JSON.stringify(signaturesObj, null, 2));
    } catch (error) {
      console.error('❌ Failed to persist console errors:', error);
    }
  }

  public getErrorsByCategory(category: string): ConsoleError[] {
    return this.errors.filter(error => error.category === category);
  }

  public getErrorsBySeverity(severity: string): ConsoleError[] {
    return this.errors.filter(error => error.severity === severity);
  }

  public getErrorsByComponent(component: string): ConsoleError[] {
    return this.errors.filter(error => error.context.component === component);
  }

  public getFrequentErrorSignatures(minFrequency: number = 2): RefactoringErrorSignature[] {
  ...
  public exportToNeuralTraining(): string {
    ...
    const exportPath = join(this.dataDir, 'neural-training-console-errors.json');
  ...
  private groupBy(field: keyof ConsoleError): Record<string, number> {
  ...
  private isRefactoringError(error: ConsoleError): boolean {
    return error.category === 'refactoring' || 
           this.refactoringPatterns.some(pattern => pattern.test(error.message));
  }

  private generatePreventionStrategy(error: ConsoleError): string {
  ...
  public generateConsoleErrorReport(): string {
    const report = {
      summary: {
        totalErrors: this.errors.length,
    ...
    const reportPath = join(this.dataDir, `console-error-report-${Date.now()}.json`);
  ...
  private generateConsoleRecommendations(): string[] {
