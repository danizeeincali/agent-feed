...
export class ClaudeProcessIOAutomatedResolution {
  ...
  private setupPrintFlagStrategies(): void {
  ...
  private setupInteractiveModeStrategies(): void {
    const strategies: ResolutionStrategy[] = [
      {
  ...
  private setupPTYDisconnectStrategies(): void {
    const strategies: ResolutionStrategy[] = [
      {
  ...
  private setupAuthSilentStrategies(): void {
    const strategies: ResolutionStrategy[] = [
      {
  ...
  async executeResolution(alert: ClaudeProcessIOAlert): Promise<ResolutionExecution> {
  ...
  private async executeStep(step: ResolutionStep, alert: ClaudeProcessIOAlert): Promise<any> {
  ...
  generateResolutionReport(): {
    ...
    mostSuccessfulStrategies: { strategy: string; successRate: number; executions: number }[];
    recommendations: string[];
  } {
    const total = this.executionHistory.length;
    const successful = this.executionHistory.filter(exec => exec.finalResult?.success).length;
    
    const executionsByCategory: Record<string, number> = {};
...
export const claudeProcessIOResolution = new ClaudeProcessIOAutomatedResolution();