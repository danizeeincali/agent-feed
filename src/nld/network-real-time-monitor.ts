...
import { NetworkFailurePatternDetector, NetworkFailurePattern } from './network-failure-pattern-detector';
...
export class NetworkRealTimeMonitor {
  private detector: NetworkFailurePatternDetector;
  private config: NetworkMonitorConfig;
  private eventSource: EventSource | null = null;
  private alertHistory: NetworkAlert[] = [];
  ...
  constructor(config: Partial<NetworkMonitorConfig> = {}) {
    ...
    this.detector = new NetworkFailurePatternDetector();
  ...
  private setupSSEConnection(): void {
    ...
    try {
      ...
      this.eventSource.onerror = (error) => {
        ...
        this.triggerNetworkFailureAlert(
  ...
  private analyzeGlobalError(event: ErrorEvent): void {
  ...
  private analyzeUnhandledRejection(event: PromiseRejectionEvent): void {
  ...
  private recordNetworkMetrics(metrics: any): void {
  ...
  private generateAlert(alertData: any): void {
  ...
  private getAffectedEndpoints(): string[] {
    ...
    recentPatterns.forEach(p => {
      if (p.context.url) endpoints.add(p.context.url);
    });

    return Array.from(endpoints).slice(0, 10);
  }

  private generateRecommendations(alertData: any): string[] {
  ...
  private calculateSeverityFromStatus(statusCode: number): NetworkAlert['severity'] {
  ...
  private logAlert(alert: NetworkAlert): void {
    const icon = {
      'SPIKE': '📈',
      'THRESHOLD': '⏱️',
      'PATTERN': '🔍',
  ...
  private broadcastAlert(alert: NetworkAlert): void {
  ...
  private generateBatchReport(): void {
    ...
    const recentAlerts = this.alertHistory.filter(
      alert => alert.timestamp >= intervalStart
    );

    const recentPatterns = this.detector.getPatterns().filter(
      pattern => pattern.timestamp >= intervalStart
    );
  ...
  private generateSummary(alerts: NetworkAlert[], patterns: NetworkFailurePattern[]): any {
  ...
  private exportBatchForTraining(report: any): void {
  ...
  // Public API
  public getRecentAlerts(limit: number = 50): NetworkAlert[] {
...
if (typeof window !== 'undefined') {
  (window as any).NLD_NetworkMonitor = new NetworkRealTimeMonitor();
}