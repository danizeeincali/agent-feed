 ...
 */
export class APIVersioningAntiPatternsDatabase {
    private patterns: Map<string, APIVersioningAntiPattern> = new Map();
    private databasePath: string;
    
    constructor(databasePath: string = '/workspaces/agent-feed/src/nld/patterns') {
     ...
     */
    private addCoreAntiPatterns(): void {
...
app.get('/api/v2/claude/metrics', metricsHandler);`
  ...
  test('related endpoints use same version', () => {
     ...
     */
    public addPattern(pattern: APIVersioningAntiPattern): void {
     ...
     * Get pattern by ID
     */
    public getPattern(id: string): APIVersioningAntiPattern | undefined {
     ...
     */
    public getPatternsByCategory(category: string): APIVersioningAntiPattern[] {
     ...
     */
    public getPatternsBySeverity(severity: string): APIVersioningAntiPattern[] {
        return Array.from(this.patterns.values())
     ...
     */
    public searchBySymptoms(searchTerm: string): APIVersioningAntiPattern[] {
        const term = searchTerm.toLowerCase();
        return Array.from(this.patterns.values())
            .filter(pattern => 
                pattern.symptoms.some(symptom => symptom.toLowerCase().includes(term)) ||
     ...
     */
    public generateDetectionRules(): any {
     ...
     */
    public generatePreventionPlaybook(): any {
        const allStrategies = Array.from(this.patterns.values())
            .flatMap(pattern => 
                pattern.prevention.map(strategy => ({
                    ...strategy,
    ...
    private loadExistingPatterns(): void {
        try {
            const dbFile = join(this.databasePath, 'api-versioning-anti-patterns.json');
            if (existsSync(dbFile)) {
                const data = readFileSync(dbFile, 'utf-8');
                const patterns = JSON.parse(data);
                
                patterns.forEach((pattern: APIVersioningAntiPattern) => {
                    this.patterns.set(pattern.id, pattern);
                });
    ...
    private saveDatabase(): void {
        try {
            const dbFile = join(this.databasePath, 'api-versioning-anti-patterns.json');
            const allPatterns = Array.from(this.patterns.values());
            writeFileSync(dbFile, JSON.stringify(allPatterns, null, 2));
...
export const apiVersioningAntiPatternsDatabase = new APIVersioningAntiPatternsDatabase();