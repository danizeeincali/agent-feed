...
export class AntiPatternsDatabase {
   ...
   */
  getMostCommonPatterns(limit = 10): AntiPattern[] {
    return Array.from(this.patterns.values())
      .sort((a, b) => b.occurrenceCount - a.occurrenceCount)
      .slice(0, limit);
  }
   ...
   */
  private generateRecommendedActions(patterns: AntiPattern[]): string[] {
   ...
   */
  private identifyTDDGaps(patterns: AntiPattern[]): any {
    const gaps = {
      missingUnitTests: new Set<string>(),
      missingIntegrationTests: new Set<string>(),
      missingValidationTests: new Set<string>()
    };
...
// Global instance
export const antiPatternsDatabase = new AntiPatternsDatabase();