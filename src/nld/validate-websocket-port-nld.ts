...
import { 
  ...
  calculateWebSocketFailureProbability,
  generateWebSocketTDDTests,
  type WebSocketFailurePredictionFeatures
} from './neural-training/websocket-port-failure-export';
import { websocketPortTDDSuite } from './prevention/websocket-port-tdd-prevention';
 ...
 */
export async function validateWebSocketPortNLD(): Promise<ValidationResult> {
 ...
 */
async function validatePatternDatabase(): Promise<boolean> {
  try {
 ...
 */
function validateNeuralTrainingExport(): boolean {
  try {
    // Validate export structure
    if (!claudeFlowNeuralExport.trainingData || claudeFlowNeuralExport.trainingData.length === 0) {
      console.error('❌ No training data available');
      return false;
    }
    
    // Validate prediction function
    const testFeatures: WebSocketFailurePredictionFeatures = {
    ...
    const failureProbability = calculateWebSocketFailureProbability(testFeatures);
    ...
    const tddTests = generateWebSocketTDDTests();
    ...
    console.log(`  ✓ Training samples: ${claudeFlowNeuralExport.trainingData.length}`);
 ...
 */
function validateTDDStrategies(): boolean {
  try {
    if (!websocketPortTDDSuite.strategies || websocketPortTDDSuite.strategies.length === 0) {
      console.error('❌ No TDD strategies available');
      return false;
    }
    
    // Validate each strategy
    for (const strategy of websocketPortTDDSuite.strategies) {
    ...
    console.log(`  ✓ Strategy count: ${websocketPortTDDSuite.strategies.length}`);
    console.log(`  ✓ Overall effectiveness: ${(websocketPortTDDSuite.totalEffectivenessScore * 100).toFixed(1)}%`);
    console.log(`  ✓ Estimated implementation time: ${websocketPortTDDSuite.estimatedImplementationTime.total}`);
...
function calculateOverallEffectiveness(): number {
  ...
  const tddScore = websocketPortTDDSuite.totalEffectivenessScore;
 ...
 */
export async function demonstrateNeuralTraining(): Promise<void> {
  ...
  for (const scenario of scenarios) {
    const probability = calculateWebSocketFailureProbability(scenario.features);
