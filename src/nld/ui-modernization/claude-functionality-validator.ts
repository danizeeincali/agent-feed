...
export class ClaudeFunctionalityValidator extends EventEmitter {
  private validationResults: ClaudeValidationResult[] = [];
  private lastValidation: number = 0;
  private validationInterval: NodeJS.Timeout | null = null;
  private criticalFailures: Set<string> = new Set();
  ...
  public async runFullValidation(): Promise<ClaudeFunctionality> {
    console.log('[NLD] Running comprehensive Claude functionality validation');

    const results: ClaudeFunctionality = {
  ...
  private async validateProcessSpawning(): Promise<boolean> {
    try {
      ...
      for (const button of claudeButtons) {
        const hasClickHandler = this.checkButtonClickHandler(button as HTMLButtonElement);
        if (!hasClickHandler) {
  ...
  private async validateButtonHandlers(): Promise<boolean> {
    try {
      ...
      if (successRate < 0.8) { // At least 80% of buttons should work
        this.recordValidationResult('buttonHandlers', false, 
          `Only ${Math.round(successRate * 100)}% of buttons have working handlers`);
        return false;
      }
  ...
  private async validateInstanceCreation(): Promise<boolean> {
    try {
      ...
      if (!testResponse.ok) {
        this.recordValidationResult('instanceCreation', false, 
          `Instance API not accessible: ${testResponse.status}`);
        return false;
      }
  ...
  private async validateTerminalConnection(): Promise<boolean> {
    try {
  ...
  private async validateSSEStreaming(): Promise<boolean> {
    try {
      ...
      if (sseTestResponse && !sseTestResponse.ok && sseTestResponse.status !== 404) {
        this.recordValidationResult('sseStreaming', false, 
          `SSE endpoint error: ${sseTestResponse.status}`);
        return false;
      }
  ...
  private checkButtonClickHandler(button: HTMLButtonElement): boolean {
  ...
  private isLoadingState(button: HTMLButtonElement): boolean {
  ...
  private checkForReactContext(contextName: string): boolean {
  ...
  private checkForReactHook(hookName: string): boolean {
    // This is a simplified check - in a real implementation, you'd need
    // to integrate with React DevTools or use other mechanisms
    const scriptTags = document.querySelectorAll('script');
    for (const script of scriptTags) {
  ...
  public getValidationHistory(): ClaudeValidationResult[] {
  ...
  public getLastValidationTime(): number {
    return this.lastValidation;
  }

  public async repairClaudeFunctionality(): Promise<boolean> {
  ...
  private async restoreButtonHandlers(): Promise<void> {
    const claudeButtons = document.querySelectorAll('button[data-claude-action]');
  ...
  private async restoreInstanceCreation(): Promise<void> {
  ...
  private async restoreTerminalConnection(): Promise<void> {
  ...
  private simulateClaudeAction(action: string, button: HTMLButtonElement): void {
  ...
  public generateClaudeHealthReport(): string {
...
${recentResults.slice(-5).map(result => 
  `${result.passed ? '✅' : '❌'} ${result.functionality}: ${result.error || 'OK'}`
).join('\n')}
...
export const claudeFunctionalityValidator = new ClaudeFunctionalityValidator();