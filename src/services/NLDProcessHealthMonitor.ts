...
export class NLDProcessHealthMonitor extends EventEmitter {
  private processMap: Map<string, ProcessHealthMetrics> = new Map();
  private healthCheckInterval: NodeJS.Timeout | null = null;
  private alertHistory: NLDAlertData[] = [];
   ...
   */
  private checkProcessLifecycle(instanceId: string, processInfo: ProcessHealthMetrics): void {
    if (!processInfo.pid || processInfo.status === 'stopped') return;
   ...
   */
  private checkIOCommunication(instanceId: string, processInfo: ProcessHealthMetrics): void {
    const now = Date.now();
   ...
   */
  private checkResourceLeaks(instanceId: string, processInfo: ProcessHealthMetrics): void {
  ...
  registerProcess(instanceId: string, process: ChildProcess): ProcessHealthMetrics {
    const metrics: ProcessHealthMetrics = {
   ...
   */
  private attachProcessMonitoring(instanceId: string, process: ChildProcess, metrics: ProcessHealthMetrics): void {
    // Monitor stdout
   ...
   */
  async spawnClaudeWithFallback(instanceId: string, command: string, args: string[], options: any): Promise<ChildProcess> {
   ...
   */
  private validateSpawnPrerequisites(command: string, workingDirectory: string): void {
  ...
  private handleSpawnFailure(
    ...
    if (!options.shell) {
      setTimeout(() => {
        try {
          const fallbackProcess = spawn(command, args, { ...options, shell: true });
    ...
    if (options.cwd !== fallbackDir && fs.existsSync(fallbackDir)) {
      setTimeout(() => {
        try {
          const fallbackProcess = spawn(command, args, { ...options, cwd: fallbackDir });
          this.registerProcess(instanceId + '_fallback', fallbackProcess);
          resolve(fallbackProcess);
        } catch (error) {
   ...
   * Send NLD Alert
   */
  private nldAlert(pattern: ProcessFailurePattern, context: any): void {
    const alert: NLDAlertData = {
      pattern,
   ...
   */
  private determineSeverity(pattern: ProcessFailurePattern, context: any): 'low' | 'medium' | 'high' | 'critical' {
    switch (pattern) {
   ...
   */
  private getResolutionStrategy(pattern: ProcessFailurePattern): string {
    const strategies = {
...
// Singleton instance
export const nldProcessMonitor = new NLDProcessHealthMonitor();

// Cleanup on process exit
process.on('SIGINT', () => {
  nldProcessMonitor.stopMonitoring();
  process.exit(0);
});
