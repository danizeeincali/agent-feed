...
import ClaudeProcessManager, { ClaudeInstanceStatus } from './ClaudeProcessManager';
...
export class HealthMonitor extends EventEmitter {
   ...
   */
  private async checkInstanceHealth(instanceStatus: ClaudeInstanceStatus): Promise<void> {
  ...
  private async calculateHealthMetrics(
    ...
    if (!isHealthy) {
      ...
      responseTime > this.config.healthThresholds.responseTime.critical ||
      errorRate > this.config.healthThresholds.errorRate.critical
    ) {
      status = 'critical';
    } else if (
      responseTime > this.config.healthThresholds.responseTime.warning ||
   ...
   */
  private storeHealthMetrics(instanceId: string, metrics: HealthMetrics): void {
  ...
  private async evaluateHealthThresholds(metrics: HealthMetrics): Promise<void> {
    ...
    if (metrics.responseTime > this.config.healthThresholds.responseTime.critical) {
      alerts.push(this.createAlert(
        ...
        'critical',
        metrics.instanceId,
        `Critical response time: ${metrics.responseTime.toFixed(2)}ms`
      ));
      ...
      alerts.push(this.createAlert(
        ...
        'high',
        metrics.instanceId,
        `High response time: ${metrics.responseTime.toFixed(2)}ms`
      ));
    }
   ...
   */
  private async attemptRecovery(instanceId: string, metrics: HealthMetrics): Promise<void> {
    const recentActions = this.recoveryActions.filter(
   ...
   */
  private async escalateRecovery(instanceId: string, metrics: HealthMetrics): Promise<void> {
    ...
    const alert = this.createAlert(
      ...
      'critical',
      instanceId,
      `Instance requires manual intervention - automatic recovery failed`
    );
   ...
   * Check if alert condition is resolved
   */
  private isAlertResolved(alert: HealthAlert, metrics: HealthMetrics): boolean {
    switch (alert.type) {
      case 'performance':
  ...
  private cleanupOldData(): void {
    ...
    for (const [instanceId, history] of this.healthMetrics) {
      const filteredHistory = history.filter(metrics => 
        metrics.timestamp.getTime() > cutoffTime
      );
   ...
   */
  getMetricsHistory(instanceId: string, limit = 50): HealthMetrics[] {
    const history = this.healthMetrics.get(instanceId) || [];
   ...
   */
  getRecoveryHistory(limit = 100): RecoveryAction[] {
