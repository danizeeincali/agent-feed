 ...
 */
export class ClaudeSessionsDB {
  /**
   * Create a new Claude session record
   */
  static async createSession(session: ClaudeSession): Promise<ClaudeSessionRecord> {
    try {
      const query = `
      ...
      logger.info(`Created Claude session record: ${session.id}`);
      
      return result.rows[0];
    } catch (error) {
      logger.error('Failed to create Claude session record:', error);
      throw error;
    }
  }
   ...
   */
  static async updateSession(sessionId: string, updates: Partial<ClaudeSession>): Promise<ClaudeSessionRecord | null> {
    try {
      ...
      if (result.rows.length > 0) {
        logger.debug(`Updated Claude session record: ${sessionId}`);
        return result.rows[0];
      }
      
      return null;
    } catch (error) {
      logger.error('Failed to update Claude session record:', error);
      throw error;
    }
  }
   ...
   */
  static async getSession(sessionId: string): Promise<ClaudeSessionRecord | null> {
    try {
      ...
      logger.error('Failed to get Claude session record:', error);
      throw error;
    }
  }
   ...
   */
  static async getUserSessions(userId: string, limit = 50, offset = 0): Promise<ClaudeSessionRecord[]> {
    try {
      const query = `
      ...
      logger.error('Failed to get user Claude sessions:', error);
      throw error;
    }
  }
  ...
  static async deleteSession(sessionId: string): Promise<boolean> {
    try {
      ...
      logger.info(`Deleted Claude session record: ${sessionId}`);
      return result.rowCount > 0;
    } catch (error) {
      logger.error('Failed to delete Claude session record:', error);
      throw error;
    }
  }
   ...
   */
  static async saveAgent(agent: ClaudeAgent, sessionId: string): Promise<void> {
    try {
      const query = `
        INSERT INTO agents (
      ...
      logger.debug(`Saved Claude agent: ${agent.id}`);
    } catch (error) {
      logger.error('Failed to save Claude agent:', error);
      throw error;
    }
  }
   ...
   */
  static async updateAgent(agentId: string, updates: Partial<ClaudeAgent>): Promise<void> {
    try {
      const setParts: string[] = [];
      const values: any[] = [];
      let paramCount = 1;
      ...
      await db.query(query, values);
      logger.debug(`Updated Claude agent: ${agentId}`);
    } catch (error) {
      logger.error('Failed to update Claude agent:', error);
      throw error;
    }
  }
   ...
   */
  static async saveTask(task: ClaudeTask): Promise<void> {
    try {
      ...
      await db.query(query, values);
      logger.debug(`Saved Claude task: ${task.id}`);
    } catch (error) {
      logger.error('Failed to save Claude task:', error);
      throw error;
    }
  }
   ...
   * Update task status
   */
  static async updateTask(taskId: string, updates: Partial<ClaudeTask>): Promise<void> {
    try {
      const setParts: string[] = [];
      const values: any[] = [];
      let paramCount = 1;
      ...
      if (updates.output || updates.startedAt || updates.agentId) {
        ...
        if (currentTask) {
          ...
          if (updates.output) resultData.output = updates.output;
          if (updates.startedAt) resultData.startedAt = updates.startedAt;
          if (updates.agentId) resultData.agentId = updates.agentId;
      ...
      await db.query(query, values);
      logger.debug(`Updated Claude task: ${taskId}`);
    } catch (error) {
      logger.error('Failed to update Claude task:', error);
      throw error;
    }
  }
   ...
   */
  static async cleanupOldSessions(olderThanDays = 30): Promise<number> {
    try {
      const query = `
      ...
      if (deletedCount > 0) {
        logger.info(`Cleaned up ${deletedCount} old Claude sessions`);
      }
...
export const claudeSessionsPersistence = {
  async syncSession(session: ClaudeSession): Promise<void> {
    try {
      ...
      logger.error('Failed to sync Claude session:', error);
    }
  },

  async syncAgent(agent: ClaudeAgent, sessionId: string): Promise<void> {
    try {
      await ClaudeSessionsDB.saveAgent(agent, sessionId);
    } catch (error) {
      logger.error('Failed to sync Claude agent:', error);
    }
  },

  async syncTask(task: ClaudeTask): Promise<void> {
    try {
      await ClaudeSessionsDB.saveTask(task);
    } catch (error) {
      logger.error('Failed to sync Claude task:', error);
    }
  },

  async loadUserSessions(userId: string): Promise<ClaudeSessionRecord[]> {
    try {
