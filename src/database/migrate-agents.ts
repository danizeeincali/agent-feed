...
async function createAgentsTable() {
  try {
    ...
    if (tableCheck.rows[0].exists) {
      logger.info('Agents table already exists');
      return;
    }
    ...
    logger.info('Sample agents created successfully');

  } catch (error) {
    logger.error('Failed to create agents table:', error);
    process.exit(1);
  } finally {
    await db.close();
  }
