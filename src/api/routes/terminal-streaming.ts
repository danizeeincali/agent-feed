...
const requireTerminalAuth = (req: Request, res: Response, next: Function) => {
  ...
  (req as any).userId = 'demo-user'; // Replace with actual user extraction
  next();
};
...
router.get('/stats', async (req: Request, res: Response) => {
  try {
    ...
    logger.info('Terminal stats requested', { userId: (req as any).userId });
...
router.get('/sessions', async (req: Request, res: Response) => {
  try {
    ...
    logger.info('Terminal sessions listed', { userId, sessionCount: sessions.totalCount });
...
router.delete('/sessions/:sessionId', async (req: Request, res: Response) => {
  try {
    ...
    logger.info('Terminal session kill requested', { sessionId, userId });
...
router.post('/sessions', async (req: Request, res: Response) => {
  try {
    ...
    logger.info('Terminal session creation requested via HTTP', { 
      sessionId, 
      userId, 
    ...
    logger.error('Failed to create terminal session via HTTP', { 
      error: error.message, 
      userId: (req as any).userId 
    });
...
router.get('/sessions/:sessionId', async (req: Request, res: Response) => {
  try {
    ...
    logger.info('Terminal session details requested', { sessionId, userId });
    ...
    logger.error('Failed to get terminal session details', { 
      error: error.message, 
      sessionId: req.params.sessionId,
      userId: (req as any).userId 
    });
...
router.post('/sessions/:sessionId/input', async (req: Request, res: Response) => {
  try {
    ...
    logger.info('Terminal input sent via HTTP', { 
      sessionId, 
      userId, 
    ...
    logger.error('Failed to send terminal input via HTTP', { 
      error: error.message, 
      sessionId: req.params.sessionId,
      userId: (req as any).userId 
    });
...
router.get('/health', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Terminal health check failed', { error: error.message });
...
router.get('/config', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to get terminal config', { error: error.message });
