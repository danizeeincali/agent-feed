...
router.get('/status', (req, res) => {
  try {
    const serviceStatus = prodClaudeService.getStatus();
    ...
    logger.error('Error getting production Claude status:', error);
    res.status(500).json({
      success: false,
...
router.post('/start', async (req, res) => {
  try {
    ...
    res.json({
      ...
      timestamp: new Date().toISOString(),
      status: prodClaudeService.getStatus()
    });
  } catch (error) {
    logger.error('Error starting production Claude service:', error);
    res.status(500).json({
      success: false,
...
router.post('/stop', async (req, res) => {
  try {
    ...
    logger.error('Error stopping production Claude service:', error);
    res.status(500).json({
      success: false,
...
router.post('/message', async (req, res) => {
  try {
    ...
    const success = hubServer.sendToInstance('prod-claude', {
      id: messageId,
      type,
      from: 'api',
      payload,
    ...
    logger.error('Error sending message to production Claude:', error);
    res.status(500).json({
      success: false,
...
router.get('/hub/instances/:type', (req, res) => {
  try {
    ...
    const instances = hubServer.getInstancesByType(type);
    ...
    logger.error('Error getting instances by type:', error);
    res.status(500).json({
      success: false,
...
router.post('/hub/send/:instanceId', (req, res) => {
  try {
    ...
    const success = hubServer.sendToInstance(instanceId, message);
  ...
  try {
    ...
    res.status(isHealthy ? 200 : 503).json({
      ...
      checks: {
        ...
        hubRunning: hubServer.getStatus().running
      }
    });
  } catch (error) {
