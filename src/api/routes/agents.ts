...
const createAgentSchema = z.object({
  name: z.string().min(1).max(255).regex(/^[a-z0-9-]+$/, 'Agent name must be lowercase with hyphens only'),
  display_name: z.string().min(1).max(255),
  description: z.string().min(1),
  system_prompt: z.string().min(1),
  avatar_color: z.string().regex(/^#[0-9A-Fa-f]{6}$/, 'Avatar color must be a valid hex color'),
...
router.post('/', authenticateToken, validateRequest(createAgentSchema), async (req, res) => {
  try {
    ...
    const existingAgent = await db.query(
      'SELECT id FROM agents WHERE user_id = $1 AND name = $2',
      [userId, agentData.name]
    );
    ...
    logger.info('Agent created', { agentId, userId, name: agentData.name });
...
router.put('/:id', authenticateToken, validateRequest(updateAgentSchema), async (req, res) => {
  try {
    const userId = (req as any).user.id;
    const agentId = req.params.id;
    ...
    if (updates.name) {
      const duplicateCheck = await db.query(
        'SELECT id FROM agents WHERE user_id = $1 AND name = $2 AND id != $3',
        [userId, updates.name, agentId]
      );
...
router.patch('/:id/status', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await db.query(query, [status, new Date().toISOString(), agentId, userId]);
    ...
    logger.info('Agent status updated', { agentId, userId, status });
...
router.patch('/bulk', authenticateToken, validateRequest(bulkActionSchema), async (req, res) => {
  try {
    const userId = (req as any).user.id;
    ...
    logger.info('Bulk agent operation completed', { 
      userId, 
      action, 
...
router.post('/:id/test', authenticateToken, validateRequest(testAgentSchema), async (req, res) => {
  try {
    const userId = (req as any).user.id;
    const agentId = req.params.id;
    ...
    const agentResult = await db.query(
      'SELECT id, name, display_name, system_prompt, status FROM agents WHERE id = $1 AND user_id = $2',
      [agentId, userId]
    );
    ...
    await db.query(
      ...
      ['testing', new Date().toISOString(), agentId]
    );
    ...
    await db.query(
      'UPDATE agents SET status = $1, performance_metrics = $2, updated_at = $3 WHERE id = $4',
      ['active', JSON.stringify(updatedMetrics), new Date().toISOString(), agentId]
    );
    
    logger.info('Agent test completed', { agentId, userId, responseTime });
    ...
    try {
      await db.query(
        'UPDATE agents SET status = $1, updated_at = $2 WHERE id = $3',
        ['error', new Date().toISOString(), req.params.id]
      );
    } catch (updateError) {
      logger.error('Failed to update agent status after test error:', updateError);
    }
...
router.post('/:id/heartbeat', authenticateToken, async (req, res) => {
  try {
    ...
    logger.error('Failed to record agent heartbeat:', error);
    res.status(500).json({
      success: false,
      error: {
