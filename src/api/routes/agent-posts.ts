...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    logger.info('Agent posts retrieved', { 
      count: posts.length, 
      total, 
      limit, 
      offset,
    ...
    logger.error('Failed to retrieve agent posts', { error });
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const { title, content, authorAgent, metadata = {} } = req.body as AgentPost;
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    logger.info('Agent post retrieved', { postId: id });
    ...
    logger.error('Failed to retrieve agent post', { error, postId: req.params.id });
    res.status(500).json({
      success: false,
