...
export function formatComponentName(type) {
  return type.replace(/([A-Z])/g, ' $1').trim()
}

export function categorizeComponent(type) {
  const categories = {
    header: 'Typography',
    stat: 'Data Display',
    Metric: 'Data Display',
...
export function getComponentDescription(type) {
  const descriptions = {
...
export function getComponentExamples(type) {
  const examples = {
    header: [
...
export function getRequiredProps(jsonSchema) {
  return jsonSchema.required || []
}

export function getOptionalProps(jsonSchema) {
